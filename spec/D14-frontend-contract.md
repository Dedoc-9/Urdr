...
| Procedural generator — terrain (`tools/terrain/heightfield.py`) | seed + params | SHA-seeded lattice noise → Q16 quintic FBM → exact rescale; own canon `URDRHF1` gated (`terrain` stage: goldens ×2, validity, defect, `TERRAIN-REFUSE`) | `MEASURED` — URDRHF1 canon AND the URDROBJ2 admission (`terrain_bridge.py`, T2): OWN canon ≡ `canon_ref` on pinned grids, provenance-inert identity, max-first canon defect caught, `check_design` ADMIT; gate rows `terrain:object*`. Cross-placement (§3 step 6) escalates with importance, as ever |
