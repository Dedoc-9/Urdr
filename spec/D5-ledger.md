...
| foreign_rust_kernel | SPECULATIVE / N/A | Can an *independent* Rust kernel (`urdr-core-rs`) reproduce the reference digest on the corpus? | Rust placement ≡ reference placement | `URDR-RUST-DIVERGENCE` | a Rust kernel matches canon+digest on the pinned corpus **and** a deliberate Rust defect is caught by the harness (`tools/foreign_placement/`). **Stage-4 target now FROZEN** — the minimal kernel contract, the `canon→SHA-256` byte grammar, and the conformance vectors are pinned in `spec/D8-portable-kernel.md` + `tools/foreign_placement/conformance.txt` (MEASURED from ☉); the remaining step is an independent `urdr-core-rs` built and run on a cargo host (this sandbox has no rustc/cargo, no root) | 0 |
