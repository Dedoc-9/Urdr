...
**Cross-region structural anti-cheat (URDRWARD2) — T3.25, MMO Stage E (the D+E synthesis) — MEASURED (merged-authority admission).** `tools/terrain/crosswarden.py` + gate stage `crosswarden`: the seamless handoff seam (URDRHAND1) is made CHEAT-PROOF. A claimed crossing is certified against the MERGED authority — `hand.merge`, `F_A` west of the split, `F_B` east — the SAME merge the handoff itself produces, not against either shard's stale view of its neighbor. `warden` (URDRWARD1) turned reconstruct-or-refuse against the cheater on ONE field; `crosswarden` turns it against the cheater who exploits the REGION BOUNDARY. THE EXPLOIT CLASS: a shard holds only its own field and a stale copy of its neighbor's, so shard A — checking a crossing against `F_A` alone — ADMITS a move that is only legal because A's copy of the east region is stale (flat where B has since raised a wall). THE HEADLINE (the reason the slice exists): A SHARD-LOCAL WARDEN IS INSUFFICIENT. On the pinned world (flat `F_A`; `F_B` with an impassable wall at x=12, east of the synced band [6,10) around split=8), the exploit trajectory (a sprint east across the seam and through B's wall) and the exploit position (a cell beyond B's wall, claimed reachable from a west anchor) BOTH ADMIT under a shard-local `warden.admit_trajectory(F_A, …)` / `admit_position(F_A, …)` — the local check passes the cheat — while `crosswarden` REFUSES both with `WARD-TUNNEL` / `WARD-UNREACH`, because the merge carries B's real wall. Only the merged-authority warden catches it. TWO CERTIFICATES, against the merge, both exact and division-free: KINEMATIC (each claimed step legal under `F_merged`; a step onto B's wall is `WARD-TUNNEL`, a diagonal or > 2-cell jump `WARD-TELEPORT`) and TOPOLOGICAL (β₀ = rank H₀ of the MERGED walkable graph; B's wall raises the merged component count above the stale shard's — β₀(F_merged) = 3 vs β₀(F_A) = 1 — so a position beyond the wall is in a different component from the west anchor, `WARD-UNREACH` from the merged field alone). THE STAGE D ↔ E TIE: an honest `hand.handoff` trajectory admits under the cross-region warden AND equals `hand.merged_glide` bit-for-bit, so the warden certifies against exactly the world the handoff traverses. Precondition: the shards must be SYNCED on the boundary band (`hand.seam_ok`); a desynced band has no canonical merge and is a `WARD-SEAM` refusal. Two pinned scenes: `honest_cross` (a seam crossing that stops before the wall, admitted), `seam_tunnel` (the through-wall sprint, refused). Four rows: `crosswarden:scenes`, `crosswarden-kinematic` (honest crossing admits; through-wall sprint is WARD-TUNNEL; honest handoff admits and equals merged_glide), `crosswarden-topological` (merge == hand.merge; β₀ rises 1 → 3; beyond-wall position WARD-UNREACH), `crosswarden-insufficient` (the shard-local warden admits both exploits crosswarden refuses; desync is WARD-SEAM; 4/4 typed WARD-REFUSE sub-codes). Red-first `tests/test_crosswarden.py` (10 falsifiers). Unit falsifiers 780 → 790; rows 494 → 498. Adds NO placement (composes `hand` + `warden` + `glide`). GRADE: the merged-authority admission/refusal, the shard-local INSUFFICIENCY witness, the merged-topology non-vacuity (β₀ genuinely rises across the merge), the honest-handoff-admits-and-equals-merged tie, and their determinism are MEASURED (exact, reproducible, a defect diverges). `does_not_show`: DIRECTED reachability (UNDIRECTED components, as URDRWARD1 — a follow-on); the network TRANSPORT of the neighbor field (who ships `F_B`); the URDRPD1 homology CROSS-PLACEMENT (β₀ computed directly here); NON-MONOTONE crossings (this certifies the east crossing, as `hand` does); and any WALL-CLOCK cost (`NOT_MEASURED` until a sealed bench, Stage H). The seam is now both invisible (URDRHAND1: byte-exact) and inviolable (URDRWARD2: a crossing that could not have happened is a certified refusal against the merged world) — and, decisively, a per-shard warden alone would have let the boundary exploit through.

**Directed-reachability structural anti-cheat (URDRWARD3) — T3.26, MMO Stage E — MEASURED (directed reachability, one-way terrain).** `tools/terrain/dirward.py` + gate stage `dirward`: the topological anti-cheat refined from UNDIRECTED reachability (URDRWARD1/2) to DIRECTED, so a ONE-WAY cliff is modelled honestly. `warden` joined adjacent cells with an UNDIRECTED edge — a step legal BOTH ways (|Δground| ≤ MAX_STEP) — which is blind to direction: a cliff you can descend but not climb has |Δ| > MAX_STEP, so the undirected warden sees NO edge and treats the one-way cliff as a SOLID WALL. TWO THINGS IT GETS WRONG (both measured): (1) IT FALSE-REFUSES THE LEGAL DESCENT — an actor drops off a ledge (Δground = -100 ≤ MAX_STEP, legal), but `warden.admit_position(top, bottom)` sees no undirected edge and REFUSES WARD-UNREACH, kicking an honest player; `dirward.admit_move(top, bottom)` ADMITS. (2) IT CANNOT DISTINGUISH A ONE-WAY CLIFF FROM A WALL — for the illegal climb bottom -> top AND for a genuine impassable wall, the undirected warden returns the SAME WARD-UNREACH; `dirward` separates them (the climb is WARD-ONEWAY — you may go the other way — the wall is WARD-UNREACH). THE INVARIANT: the DIRECTED walkable graph has an edge a -> b iff field[b] - field[a] ≤ MAX_STEP (descending always legal, ascending only within the step law); `can_reach` is its transitive closure, ASYMMETRIC — on the pinned plateau-over-plain world (west x<8 at height 100, east x≥8 at height 0, MAX_STEP=40) can_reach(top, bottom) is true and can_reach(bottom, top) is false. `reach_asymmetry` counts the ordered pairs reachable only one way — 0 exactly when reachability is symmetric (flat/gentle terrain), positive on a cliff (16384 on the pinned world) — the live non-vacuity number. HONEST SCOPE: `num_scc` (strongly-connected = mutual-reachability classes) is the directed analog of β₀, but because a both-ways edge is a 2-cycle every undirected component sits inside one SCC, so num_scc ≤ betti0 always and EQUAL on these worlds (a strict coarsening needs a directed cycle bridging undirected components, not shown); the directed refinement rests on the reachability ASYMMETRY, not a component-count gap. The gap closed is TOPOLOGICAL not kinematic: a climb-back TRAJECTORY is already refused per-step (the uphill step is a WARD-TUNNEL); dirward closes the BARE-POSITION hole. Two pinned scenes: honest_descent (top -> bottom, admitted), climb_back (bottom -> top, WARD-ONEWAY). Four rows: `dirward:scenes`, `dirward-asymmetry` (asymmetric reach, reach_asymmetry > 0 on the cliff and 0 with num_scc == betti0 on flat), `dirward-admission` (descent WARD-OK; climb WARD-ONEWAY; wall WARD-UNREACH; glide descent admits kinematically), `dirward-insufficient` (the undirected warden false-refuses the descent AND conflates one-way with wall; dirward fixes both; typed sub-codes). Red-first `tests/test_dirward.py` (10 falsifiers). Unit falsifiers 790 -> 800; rows 498 -> 502. Adds NO placement (composes `glide` + the undirected `warden` it refines). GRADE: the directed reachability and its asymmetry, the one-way / unreachable refusals, the two undirected-warden failure witnesses, and their determinism are MEASURED (exact, reproducible, a defect diverges). `does_not_show`: the KINEMATIC trajectory case (already covered by the warden step law); a strict num_scc < betti0 coarsening (needs a bridging directed cycle, not built); variable/weighted step costs (single MAX_STEP); the URDRPD1 homology CROSS-PLACEMENT (computed directly here); and any WALL-CLOCK cost (NOT_MEASURED until Stage H). The anti-cheat now tells "descended a cliff" (legal) from "climbed back up" (impossible) — the distinction an undirected component map structurally cannot make.

**Warden beta0 cross-placed as F2-homology (URDRWARDH1) — T3.27, MMO Stage E — MEASURED (three placements).** `tools/terrain/wardhom.py` + `wardhom_c/` (C99) + `wardhom_rs/` (Rust) + gate stage `wardhom`: the wardens' topological certificate — β₀ of the walkable field, computed DIRECTLY by union-find in `warden` — IS the invariant URDRPD1 certifies as rank H₀ over 𝔽₂, and this slice wires the two together and CROSS-PLACES the walkable-graph homology in Python + C99 + Rust. THE FOLLOW-ON THE WARDENS NAMED: warden / crosswarden / dirward each computed β₀ directly and flagged "wiring homology_c / homology_rs to the warden is a named follow-on" in their does_not_show; this discharges it. THE BRIDGE: the warden's walkable field is a graph (a vertex per cell, an undirected edge between adjacent cells with |Δground| ≤ MAX_STEP); read as a 1-complex (0-simplices = cells, 1-simplices = legal-step edges), β₀ = n₀ - rank(∂₁) = the component count — exactly `warden.betti0`, but by 𝔽₂ boundary-matrix rank instead of union-find — and β₁ = n₁ - rank(∂₁) = the walkable graph's independent cycles. THE KEYSTONE (MEASURED): TWO independent methods agree — `warden.betti0` (union-find) == URDRPD1 𝔽₂-rank β₀ — on every pinned world AND the 16×16 warden barrier; and the 𝔽₂-rank computation is CROSS-PLACED: `wardhom_c/wardhom.c` (own 𝔽₂ reduction + own SHA-256, `cc -O2 -std=c99 -Wall -Wextra`, zero warnings) and `wardhom_rs/wardhom.rs` (std-only, `rustc -O`) build the same walkable complex from the same field and reproduce the URDRWARDH1 digest (SHA-256 of MAGIC | name | n₀ | n₁ | rank | β₀ | β₁) BIT-FOR-BIT — verified in-session on rustc 1.95.0 / gcc 13.3. So the anti-cheat's topological certificate is no longer only a direct union-find count; it is the rank of a boundary operator over 𝔽₂, reproduced across three languages, which is the invariant the URDRPD1 detector already certifies. Three pinned 8×8 worlds (so ≤ 64 vertices — one uint64 per 𝔽₂ column in C99/Rust): barrier8 (β₀=3), cliff8 (β₀=2), flat8 (β₀=1). Gate stage `wardhom` (4 rows: scenes / tie / betti / placement) RE-COMPILES wardhom_c and wardhom_rs LIVE where a toolchain is present (SKIPPED and still one row otherwise, so the row count stays host-stable) and asserts they reproduce the goldens with `--defect` diverging. Red-first `tests/test_wardhom.py` (8 falsifiers). Unit falsifiers 800 → 808; rows 502 → 506; Rust placements 24 → 25, C99 runtimes 13 → 14 — the first placement-count bump since the terrain arc began, a genuine new cross-placement. GRADE: the union-find-vs-𝔽₂-rank agreement, the β₀ / β₁ counts, and the THREE-PLACEMENT digest parity (Python == C99 == Rust) are MEASURED (exact, reproducible, a defect diverges). `does_not_show`: β₂ and higher (the walkable graph is 1-dimensional by construction); PERSISTENCE (this is the static complex's homology, not a Rips filtration); torsion (𝔽₂ ranks only, as URDRPD1); weighted edges (a single MAX_STEP); and any WALL-CLOCK cost (NOT_MEASURED until a sealed bench, Stage H). The homology here is the walkable graph's, not a point cloud's — but it uses URDRPD1's exact 𝔽₂ machinery, so the two agree on β₀ by construction, and the warden's anti-cheat now rests on a cross-placed topological invariant rather than a single union-find pass.

**Region-boundary + directed wardens wired to homology (URDRWARDH2) — T3.28, MMO Stage E — MEASURED (cross-placed tie + a falsified bound).** Extends URDRWARDH1 from the single-field warden to the OTHER two wardens the operator named — crosswarden's REGION BOUNDARIES and dirward's DIRECTED SCCs. CROSSWARDEN (exact, cross-placed): a new pinned scene `merge8` — an 8×8 crosswarden MERGE (flat shard A, walled shard B, wall east of the seam) — is added to all three placements (Python + `wardhom_c` + `wardhom_rs`), reproducing its URDRWARDH1/2 digest bit-for-bit; and `crosswarden.cross_betti0` (union-find on the merged authority) == the F2-rank homology β₀ on `merge8` AND on the real 16×16 crosswarden world (both = 3). So the cross-region anti-cheat's β₀ is the cross-placed homology invariant, not just the single field's. DIRWARD (a BOUND, honestly): the directed strongly-connected count `dirward.num_scc` <= the undirected homology β₀, always — and the inequality is STRICT where a directed cycle fuses undirected components. The tempting "num_scc == β₀" theorem was tested and FALSIFIED (13 of 399 deterministic random fields have num_scc < β₀), so this is graded a bound, not an equality. A minimal 3×2 witness `_DIRMERGE` = ((66,3,32),(105,82,59)) at MAX_STEP=34 pins the gap: homology β₀ = 2 (two undirected components) but num_scc = 1 (a directed 6-cell cycle makes them one SCC) — the directed merge F2 homology STRUCTURALLY CANNOT SEE (homology is undirected). Gate stage `wardhom` grows to 5 rows (adds `wardhom-wardens`: the crosswarden region tie == 3, the dirward bound, and the strict 1 < 2 witness); the scenes and live-placement rows now cover 4 goldens (`merge8` included). Red-first `tests/test_wardhom.py` grows to 13 falsifiers (+5: the crosswarden 8×8 and 16×16 ties, the dirward bound, the strict-gap witness, and a `num_scc <= β₀` random-corpus check that never violates). Unit falsifiers 808 → 813; rows 506 → 507; NO placement bump (extends `wardhom_c`/`wardhom_rs` with a scene, no new binary). GRADE: the crosswarden region-boundary tie (cross-placed on `merge8`, Python on the 16×16 world), the `num_scc <= β₀` bound with its strict witness, and the digest parity are MEASURED (exact, reproducible, a defect diverges). `does_not_show`: a directed HOMOLOGY (num_scc is a directed-graph quantity, not F2 simplicial homology — this slice bounds it by the undirected β₀ and shows the gap, it does not cross-place num_scc itself); β₂+, persistence, torsion, weighted edges, wall-clock (as URDRWARDH1). The anti-cheat's undirected component count is now cross-placed across all three wardens; its directed refinement is bounded by that cross-placed invariant, with the gap made concrete.

**Certified integer-work envelope + wall-clock bench (URDROPC1) — T3.29, MMO Stage H opener — MEASURED (op-count) + MEASURED-on-named-host (wall-clock).** `tools/terrain/opcost.py` (certified) + `tools/terrain/bench.py` (standalone) + gate stage `opcost`: opens Stage H by turning the pervasive wall-clock `NOT_MEASURED` into a two-layer measurement. THE INSIGHT (from the OPODIS 2025 rollback formalization): byte-exactness makes the immersion-consistency bound δ ≤ η hold with δ = 0 exactly, so the only free variable left is LATENCY — and latency splits into two honestly-separable layers, only one of which can live in the byte-exact gate. WORK: the exact number of integer ops an operation performs, a pure function of the input, DETERMINISTIC and gate-CERTIFIED — it bounds the wall-clock (time ≤ op_count × per-op cost). WALL-CLOCK: the per-op cost on a NAMED host, non-deterministic, MEASURED-ON-NAMED-HOST, never gated (it would break the byte-identical law). THE CERTIFIED OP-COUNTS (exact, deterministic): glide_micro_count = len(micro) − 1 (bounded above by the no-wall Σ GAIT[gait]·sub; strictly less when a wall bites — non-vacuous); warden_edge_checks = (W−1)H + W(H−1) (a closed form == the adjacencies union-find examines); admit_substeps = Σ|dx|+|dy|; wardhom_columns = n1; tick_envelope = the per-tick work (glide + admit per actor). THE WORK-BUDGET REFUSE: within_budget(cost, budget) raises a typed `OPCOST-REFUSE` when the op-count exceeds a ceiling — the FIELD-REFUSE / TOPOLOGY-REFUSE law lifted from arithmetic overflow to a tick's work budget (a tick refuses rather than silently overruns; seeds the Stage-H hardening). THE BENCH: `bench.py` times the core ops and reports min/p50/p99/max ns beside the certified op-count. First sealed run (MEASURED-on-named-host — cloud host Intel Xeon @ 2.80GHz, CPython 3.11.15, Linux x86_64, 2000 iters): a warden β₀ p50 ≈ 216 µs over 480 edges (~449 ns/edge), a 3-actor tick p50 ≈ 70 µs, a single glide p50 ≈ 10 µs; re-run on any host for that host's numbers (the op_count column is identical everywhere). Five pinned op-count scenes (glide_open / glide_wall / warden_flat16 / wardhom_barrier8 / tick3_flat16) reproduce their URDROPC1 digests; gate stage `opcost` (4 rows: scenes / exact / bound / budget); red-first `tests/test_opcost.py` (9 falsifiers). Unit falsifiers 813 → 822; rows 507 → 511; NO placement change (`bench.py` is a standalone tool, not gated; Rust/C99/detectors unchanged). GRADE: the op-counts (exactness, determinism, the closed forms matching the real work, count ≤ bound with a non-vacuous wall witness, the budget refusal) are MEASURED; the bench's per-op ns are MEASURED-ON-NAMED-HOST (a bench number records its host, as the C99/Rust placements record their toolchain). `does_not_show`: determinism of the TIMES (only the op-counts reproduce); the constant factor across hosts / compilers / Python builds; cache / memory effects; and any worst-case under an adversarial scheduler (the op-count bounds the WORK, not the OS's willingness to run it). This is the honest form of a latency guarantee for a byte-exact engine: the guarantee lives in the certified op-count, the measurement in the host-tagged bench.

**Per-tick work governor (URDROPC2) — T3.30, MMO Stage H — MEASURED (live enforcement).** `tools/terrain/govern.py` + gate stage `govern`: the `opcost` envelope turned into LIVE ENFORCEMENT, and entirely self-contained — no network, no chunking, no external infrastructure. `opcost` (URDROPC1) certified the exact WORK of a tick and seeded a typed `OPCOST-REFUSE`; this makes the budget bind every tick. THE LAW: given a per-tick op-budget, `admit_tick` admits a FIFO PREFIX of the actors whose cumulative work fits and DEFERS the rest to the next tick. THREE GUARANTEES, all exact and gate-certified: (1) NEVER-OVERRUN — an admitted tick's spent work is always ≤ budget, so its wall-clock is bounded by budget × the host per-op cost (the benched number) — the live latency guarantee. (2) PROGRESS (no deadlock) — a single actor whose cost alone exceeds the budget is a hard `OPCOST-REFUSE` (an impossible request, refused outright, never left to starve a queue); with that filter every remaining actor costs ≤ budget, so the first queued actor always fits and every tick admits ≥ 1. (3) BOUNDED-WAIT (FIFO fairness) — in-order admission + guaranteed progress means `drain` serves N actors in ≤ N ticks and an actor at queue position p waits at most p ticks (no starvation). So a deferred actor is served on a later tick within a certified bound while no tick runs over budget: refuse-or-defer, never overrun; serve-in-order, never starve. `actor_cost` = one actor's `opcost` contribution (glide micro-steps + admit sub-steps); `admit_tick` is a greedy in-order bin fit; `drain` and `wait_ticks` iterate it — all exact integer op-counts, so the governor reproduces bit-for-bit. Three pinned schedules on 5 flat-world actors (per-actor cost 20): fits_one = (5,) [generous budget], split_two = (3,2), one_each = (1,1,1,1,1). Gate stage `govern` (4 rows: scenes / never-overrun / progress-wait / refuse) + red-first `tests/test_govern.py` (9 falsifiers). Unit falsifiers 822 → 831; rows 511 → 515; NO placement change (composes `opcost` + `glide`; Rust/C99/detectors unchanged). GRADE: the never-overrun bound, the single-over-budget REFUSE, progress (≥ 1 admitted/tick) and bounded-wait (≤ N ticks, FIFO), conservation (admitted + deferred = all actors, none lost / duplicated / reordered), and determinism are MEASURED (exact, reproducible, a defect diverges — a non-deferring governor overruns 100 vs a budget of 40). `does_not_show`: WALL-CLOCK enforcement (this bounds the WORK per tick; the wall-clock bound follows only via `bench.py`'s per-op cost, MEASURED-on-named-host); priority / weighted fairness (this is FIFO — a priority queue is a follow-on); pre-emption mid-actor (admitted whole or deferred whole); and any guarantee under an adversarial OS scheduler (the governor bounds the work a tick ISSUES, not the time the OS grants it). The latency envelope is now enforced, not just measured: every tick's work is capped and every actor is served within a certified bound.

**Stage-H latency guarantee COMPLETED — priority governor (URDROPC3) + rollback horizon (URDRLAT1) + composite SLO (URDRLAT2) — T3.31-33, MMO Stage H — MEASURED (a 3-in-1 pass).** Three self-contained slices that close the latency arc `opcost`/`govern` opened; each composes what exists (no network, no chunking). PRIORITY GOVERNOR (`tools/terrain/priogov.py`): the weighted-fairness follow-on `govern` named — admit the highest-EFFECTIVE-priority prefix within the tick op-budget, where effective priority = base + age_step·wait, so AGING keeps the lowest base priority from ever starving. Same never-overrun as `govern`, plus fresh-tick priority order (top priority served tick 1, lowest last) and no-starvation (every actor served <= N ticks); a single over-budget actor is `OPCOST-REFUSE`. ROLLBACK HORIZON (`tools/terrain/horizon.py`): the worst-case RECONCILE latency as a hard bound. Because URDR reconciles byte-exactly (the OPODIS immersion-consistency distance δ = 0), the only cost of a late input is HOW FAR BACK it rolls — the depth n − k that `cpredict` replays. `admit_reconcile` ADMITS iff depth <= horizon H (and `cpredict.reconstruct` lands on the authority BIT-FOR-BIT), else `HORIZON-REFUSE` (a correction older than the snapshot window, refused, not served late). So the worst-case reconcile latency is exactly H. COMPOSITE SLO (`tools/terrain/slo.py`): the end-to-end guarantee as ONE certified number — worst_case_latency = admission_wait (ceil(N / floor(budget/actor_cost)), the governor) + horizon (the rollback window); `slo_admit` refuses a config whose worst-case exceeds its target (`SLO-REFUSE`, never over-promise). SOUNDNESS is checked, not assumed: the gate confirms admission_wait UPPER-BOUNDS the governor's ACTUAL drain over a 32-config corpus (exact for equal-cost actors), so the number is a real guarantee. Nine pinned scenes; three gate stages (`priogov` / `horizon` / `slo`, 4 rows each = 12) + red-first `tests/test_priogov.py` / `test_horizon.py` / `test_slo.py` (8 falsifiers each = 24). Unit falsifiers 831 → 855; rows 515 → 527; NO placement change (compose `govern` + `cpredict` + `opcost`; Rust/C99/detectors unchanged). GRADE: never-overrun + priority + no-starvation (priogov), the depth bound + byte-exact reconstruct + `HORIZON-REFUSE` (horizon), the composite worst-case + admission-soundness + `SLO-REFUSE` (slo), and determinism are MEASURED (exact, reproducible, a defect diverges — a non-aging strict priority can starve, a non-sound admission bound would under-count the real drain). `does_not_show`: WALL-CLOCK (all bounds are in TICKS / op-counts; seconds come only via `bench.py`, MEASURED-on-named-host); priority-CLASS latency in the SLO (uses the FIFO uniform bound, not `priogov`'s per-class one — a follow-on); pre-emption mid-actor; the snapshot STORAGE cost of the H-deep window; and any guarantee under an adversarial OS scheduler. The latency envelope `opcost` measured and `govern` enforced is now COMPLETE: prioritised without starvation, bounded in its rollback window, and stated as a single worst-case SLO with a refuse — every part a certified integer bound.

**Per-class latency SLO (URDRLAT3) — T3.34, MMO Stage H — MEASURED (per-class bound, checked exact against `priogov`).** `tools/terrain/clslo.py` + gate stage `clslo`: the priority-CLASS latency the composite `slo` named in its own does_not_show ("priority-class latency — uses the FIFO uniform bound, not `priogov`'s per-class one — a follow-on"). `slo` stated ONE worst-case latency for the whole population — a uniform promise; `clslo` refines it into a PER-TIER promise and makes each tier's number accountable, so `priogov`'s weighted fairness stops being a bare mechanism and becomes a certified guarantee. THE LAW: `priogov` serves highest-base-priority first, exactly m = floor(budget/actor_cost) equal-cost actors per tick, so each priority class occupies a CONTIGUOUS block of the service order and the last actor of the class at priority p finishes at tick ceil(N≥p / m), where N≥p is the population at priority ≥ p; adding the shared rollback window H gives class_worst_case_latency(p) = ceil(N≥p / m) + H. TWO STRUCTURAL CONSEQUENCES, both certified: (1) REFINEMENT — a higher priority sees a SMALLER-or-equal cumulative population, so a tighter bound; the LOWEST class sees the whole population, so its bound is EXACTLY the composite `slo`'s uniform number (the one-class case recovers `slo` bit-for-bit). Priority buys a better latency, never a worse one. (2) PER-CLASS REFUSE — `class_slo_admit` ADMITS iff EVERY tier meets its OWN target, else `CLSLO-REFUSE` NAMING the highest-priority tier that misses (a broken promise is declined by name, never folded into an average). THE SOUNDNESS (checked, not assumed): the gate runs `priogov` over a corpus of class configs (varied populations, budgets, aging rates) and confirms ceil(N≥p / m) EQUALS the measured last-served tick of every class — EXACT for equal-cost actors, the same discipline that made the composite `slo` trustworthy, now on the harder per-class quantity; a red-first smoke test verified this over 345 checks BEFORE the digests were pinned (an off-by-one or a self-excluded population makes the measured tick diverge, and both were confirmed to redden). Three pinned scenes on premium(p3,2) / standard(p2,3) / free(p1,4), m=3, H=2: tiered_admit (premium 3, standard 4, free 5 — every tier meets, ADMIT), premium_refuse (premium's 3 > a target of 2 — `CLSLO-REFUSE:p3`), single_uniform (one class == `slo`'s 4). Gate stage `clslo` (4 rows: scenes / refinement / soundness / refuse) + red-first `tests/test_clslo.py` (8 falsifiers). Unit falsifiers 855 → 863; rows 527 → 531; NO placement change (composes `priogov` + `slo` + `horizon`; Rust/C99/detectors unchanged). GRADE: the per-class bound, its refinement (monotone in priority; one class == `slo`), its soundness against the real `priogov` drain (exact for equal-cost), the by-name `CLSLO-REFUSE`, and determinism are MEASURED (exact, reproducible, a defect diverges). `does_not_show`: WALL-CLOCK (ticks, not seconds — `bench.py` is MEASURED-on-named-host); HETEROGENEOUS per-actor costs (this bounds the equal-cost class `priogov` schedules — a non-fitting prefix that admits fewer than floor(budget/cost) is a follow-on); dynamic ARRIVALS mid-drain (the closed form is for a static population; aging still bounds the wait); pre-emption; and any guarantee under an adversarial OS scheduler. The composite SLO's single number is now a per-tier table — premium waits behind fewer actors than free, each tier's promise kept or declined by name, and every bound checked equal to the real scheduler.

**Snapshot-storage envelope (URDRLAT4) — T3.35, MMO Stage H — MEASURED (exact bytes, checked against a real serialization).** `tools/terrain/storecost.py` + gate stage `storecost`: the SPACE companion to the latency guarantee's time bounds, and the piece `horizon`'s own does_not_show deferred — "the snapshot STORAGE cost of keeping H states (an operational parameter, not certified here)." The whole Stage-H arc bounded TIME exhaustively (op-counts, per-tick budget, priority, aging, rollback window, composite and per-class SLOs) but never bounded SPACE; a byte-exact engine that promises a depth-H reconcile window must also bound the memory that window costs, or the latency promise silently rests on unbounded storage — a time bound with no space bound is half a guarantee. THE MEASURED CORE (checked against a REAL serialization, not asserted): a SNAPSHOT is the boundary state `glide` already yields — N actor poses, each (fx, fy, ground, facing) — and `serialize` is a canonical fixed-width encoding (a 4-byte actor count, then per pose 8+8+8 bytes for fx/fy/ground as signed int64 + 1 byte facing = 25 bytes), so snapshot_bytes(n) = 4 + 25·n is a CLOSED FORM the gate checks EQUALS len(serialize(state)) for real glide states over a corpus, with deserialize(serialize(state)) == state BIT-FOR-BIT and an out-of-range field a typed `STORAGE-REFUSE` (reconstruct-or-refuse, never a silent truncation — verified with non-zero/negative ground heights and near-int64 positions, and a red-first smoke test confirmed a dropped or mis-sized field moves the length before the digests were pinned). THE WINDOW (exact integer composition): a depth-H rollback window spans worst_case_window(H) + 1 = H+1 boundary snapshots, so window_storage(H, n) = (H+1)·snapshot_bytes(n) — monotone in both H and N, and tied to `horizon.worst_case_window`. THE REFUSE: within_storage_budget(bytes, budget) ADMITS iff the window fits, else `STORAGE-REFUSE` — the `opcost.within_budget` work-budget discipline lifted from time to memory (shrink H, reduce N, or raise the budget until it fits). Three pinned scenes: one_h4 (1 actor, H=4 → 5×29 = 145 bytes, ADMIT), five_h8 (5 actors, H=8 → 9×129 = 1161 bytes, ADMIT), over_budget (the same 1161 against a 1000-byte budget → STORAGE-REFUSE). Gate stage `storecost` (4 rows: scenes / soundness / window / refuse) + red-first `tests/test_storecost.py` (8 falsifiers). Unit falsifiers 863 → 871; rows 531 → 535; NO placement change (composes `glide` + `horizon` + `opcost`; Rust/C99/detectors unchanged). GRADE: the snapshot byte count and its equality to a real serialization, the byte-exact round-trip, the overflow `STORAGE-REFUSE`, the window closed form and its monotonicity, the budget `STORAGE-REFUSE`, the tie to `horizon.worst_case_window`, and determinism are MEASURED (exact, reproducible, a defect diverges); the RETENTION COUNT (H+1 snapshots for a depth-H window) is a DECLARED buffer-sizing policy — an operational parameter like rollback's K/H, never semantic — but the arithmetic composing it with the measured byte count is exact. `does_not_show`: WALL-CLOCK or bandwidth of writing snapshots (BYTES retained, not time — `bench.py` territory, MEASURED-on-named-host); COMPRESSION / delta-encoding (the raw canonical size, the honest upper bound); the FIELD's own storage (static and shared, not per-snapshot — until live world edits); and any guarantee under an adversarial allocator. The latency guarantee now has both axes: `opcost` bounded the work per tick, `storecost` bounds the memory per rollback window — time and space, each a certified integer bound with a refuse.

**Persistent snapshot checkpoint (URDRLAT5) — T3.36, MMO Stage H — MEASURED (durable round-trip through a real directory, exhaustively corruption-checked).** `tools/terrain/persist.py` + gate stage `persist`: the DURABLE realization of `storecost`'s space bound — the space bound was the prerequisite, this is what it priced. `storecost` certified the exact bytes a depth-H rollback window retains, but those bytes lived only in RAM (`rollback` keeps its canonical snapshots in memory; a process that dies loses the window the guarantee priced). `persist` closes that in-discipline gap on the membrane's own identity law (`digest = SHA-256(content)` — the `registry/pin.py` / `urdr/snapshot.py` discipline: the filename IS the digest, load verifies or refuses). THE RECORD: checkpoint(state, boundary) = MAGIC(8) | boundary(8) | storecost payload | SHA-256(32), so record_bytes(n) = 48 + snapshot_bytes(n) = 52 + 25·n is a CLOSED FORM the gate checks EQUALS len(checkpoint(real glide state)) over a corpus; the ONE digest serves three roles — integrity check, content address, on-disk filename. `restore` is bit-exact or a typed `PERSIST-REFUSE`: EVERY single-byte flip of a pinned 77-byte record, EVERY truncation, a one-byte extension, and a crafted well-digested-but-malformed payload are all checked to refuse (reconstruct-or-refuse with no undetected byte). THE MANIFEST: the window as one durable object — WIN_MAGIC | count | (boundary | record-digest)· | SHA-256, manifest_bytes(H) = 47 + 40·(H+1), boundaries strictly consecutive; a swapped, gapped, missing, or SUBSTITUTED record (valid bytes under the wrong name) refuses — enforced in TWO independent layers (`restore_window`'s digest-vs-entry check and `load_window`'s hash-to-name law), each proven non-vacuous by disabling both (the falsifier reddens) and only one (still green — defense in depth, measured). THE ENVELOPE (the realization identity): durable_window_bytes(H, n) = (H+1)·record_bytes(n) + manifest_bytes(H) EQUALS window_storage(H, n) + envelope_overhead(H) — the durable checkpoint costs exactly what `storecost` bounded plus a closed-form integrity premium (48 bytes per record + the manifest), checked EQUAL to the real bytes written, monotone in H and N, gated by `storecost.within_storage_budget` under the same `STORAGE-REFUSE` law. Three pinned scenes: one_h4_durable (1 actor, H=4 → 5×77 + 247 = 632 bytes, ADMIT), five_h8_durable (5 actors, H=8 → 9×177 + 407 = 2000 bytes, ADMIT), tampered (one flipped payload byte in a REAL record → PERSIST-REFUSE). Gate stage `persist` (4 rows: scenes / durable / window / refuse — the durable row does a REAL tempdir round-trip: filenames are digests, a re-save is byte-identical) + red-first `tests/test_persist.py` (8 falsifiers, each proven to bite via a planted defect before the digests were pinned). Unit falsifiers 871 → 879; rows 535 → 539; NO placement change (composes `storecost` + `horizon` + the membrane's digest law; Rust/C99/detectors unchanged). GRADE: the record closed form and its equality to real encodings, the bit-exact restore (including negative grounds and near-int64 positions), the exhaustive corruption/truncation refuse, the content-address law, the manifest's order/membership binding, the window-count tie to `horizon.worst_case_window(H)+1`, the realization identity, the budget refuse, and determinism (a re-save is byte-identical, same names same bytes) are MEASURED (exact, reproducible, a defect diverges); the RETENTION COUNT (H+1) stays the DECLARED buffer-sizing policy `storecost` declared, and the STORE MEDIUM (a directory of digest-named files) is a DECLARED demonstration līmes — the certified object is the BYTES; any content-addressed medium serving the same bytes inherits the guarantee. `does_not_show`: WALL-CLOCK or bandwidth of the writes (`bench.py` territory, MEASURED-on-named-host); CRASH-ATOMICITY (a torn or partial write is DETECTED on load — refused, not repaired — never PREVENTED; fsync/rename ordering is the OS's, not certified); COMPRESSION / delta-encoding (raw canonical size, the honest upper bound); CONCURRENT writers; the FIELD's own storage (static and shared — until live world edits). The Stage-H guarantee is now time-bounded, space-bounded, and durable: the window `horizon` promised, `storecost` priced, and `persist` makes survive the process — the last in-discipline piece before the scale rungs (chunked worlds, streaming load).

**Chunk loader (URDRCHK1) — T3.37, MMO Stage I opener — MEASURED (reassembly byte-for-byte with the canon tie; partial-glide equality over a boundary-tight corpus).** `tools/terrain/chunkload.py` + gate stage `chunkload`: the streaming world's first rung — the piece both `storecost` and `persist` deferred in their does_not_show ("the FIELD's own storage, static and shared"), now certified. An MMO-scale world cannot hold the whole field resident; chunkload cuts the certified terrain authority into content-addressed CHUNKS and streams them back under an EQUAL-OR-REFUSE law. THE CUT: chunk records MAGIC | kx | ky | cw | ch | cells | SHA-256 with chunk_bytes(C) = 56 + 8·C² a CLOSED FORM checked EQUAL to real records (C in the frozen {4, 8, 16, 32}; non-divisible dims refuse, never pad); the ONE digest is integrity check, content address, and filename (the `persist` law); a field manifest binds the digest grid (manifest_bytes = 55 + 32·nchunks). `reassemble` reproduces the original field BYTE-FOR-BYTE from verified chunks and the gate ties the result to the pinned URDRHF1 canon digest — cutting and streaming provably do not touch terrain identity (the D16 pattern — partition, reunify, same witness — applied to the terrain authority). EVERY single-byte flip and EVERY truncation of a pinned 184-byte C=4 record is checked to refuse. THE LOADER: `glide_partial` re-runs the frozen mover's fold against a partial residency through one abstract getter (`glide` is consumed, never edited — the `rollback` reimplement-and-pin pattern), and the gate pins its output EQUAL to `glide.glide` / `glide.glide_cells` bit-for-bit over a corpus that includes walks, sprints, wall and off-grid stops, seam crossings, subs 1/4/8/16, and a BOUNDARY-TIGHT trio (a real rise == max_step, one above, one below — planted ±1 gate-law drifts redden, proved red-first). A probe into an unloaded chunk refuses the WHOLE glide: never unloaded-terrain-as-wall, because the full mover PROBES a destination before deciding it is blocked, so a view that cannot see that cell cannot honestly reproduce the stop (a pinned seam-wall falsifier demands a chunk the actor never enters). THE DEMAND SET: `demand_chunks` records every cell the fold READS (occupied + probed, including blocked destinations) — gate-proved SUFFICIENT (a view of exactly that set reproduces the full glide) and NECESSARY (dropping ANY member refuses; an over-broad set also reddens) — "which chunks does this transcript need" has an exact certified answer, not a heuristic radius. THE ENVELOPE: field_storage(W, H, C) = nchunks·chunk_bytes(C) + manifest_bytes and resident_bytes(k, C) = k·chunk_bytes(C), checked EQUAL to real bytes, gated by `storecost.within_storage_budget` under the same `STORAGE-REFUSE` law — the static field priced exactly as the dynamic window was. Three pinned scenes: island16 (64×64 at C=16 → 16 chunks, field_storage 34231 bytes, reassembly ADMIT), walk_demand (the mountains wall walk over exactly its demand set → 1 chunk, 2104 resident bytes, trajectory-equality ADMIT witnessed by the glide digest), cold_start (an empty residency → CHUNK-REFUSE on the very first read). Gate stage `chunkload` (4 rows: scenes / reassembly / locality / refuse) + red-first `tests/test_chunkload.py` (9 falsifiers, each proven to bite via planted defects — including the corpus-hardening find that a ±1 gate drift survived the original vectors until the boundary-tight trio was added). Unit falsifiers 879 → 888; rows 539 → 543; NO placement change. GRADE: the record closed form and round-trip, the exhaustive corruption/truncation refuse, the byte-exact reassembly and its URDRHF1 tie, the partial-glide equality, the demand set's sufficiency and necessity, the store-fetch refusals (tampered / substituted / coord-forged / missing), the storage closed forms, the budget refuse, and determinism are MEASURED (exact, reproducible, a defect diverges); the CHUNK SIZE is a DECLARED operational parameter (identity and equality hold for every frozen C; which C a host picks is sizing policy, like rollback's K/H), and the residency POLICY is DECLARED (the demand set certifies what a transcript NEEDS, not what a server should cache). `does_not_show`: EVICTION (when to drop a chunk — operational); PREFETCH / async load latency (`bench.py` territory, MEASURED-on-named-host); LIVE world edits (the field is still static; a mutable chunked world is the next stage); INTEREST-relevance composition (`interest` says who must be TOLD, this says what must be LOADED); and cross-placement (Python reference only until a second placement reproduces these digests). The scale story opens with its invariant intact: a world you stream is byte-identical to the world you didn't have to.

**Resurrection law (URDRLAT6) — T3.38, MMO Stage H capstone — MEASURED (through a REAL dead-process boundary, not a simulation of one).** `tools/terrain/resurrect.py` + gate stage `resurrect`: the RECOVERY half of `persist`. `persist` proved the rollback window survives its process — a storage round-trip, bytes in, bytes out. resurrect proves the window is a recovery SUBSTRATE: the restore-and-replay law production stream processors run on (Flink's Chandy-Lamport checkpoints: restore local snapshots, replay the rewindable source), landed on URDR's terms where "exactly-once" sharpens to "the same bytes" — the tick is already a globally synchronous barrier (lockstep), the input transcript already the rewindable source (`authinput`), the persist window already the local snapshot. THE DEATH BOUNDARY IS REAL: the gate spawns a fresh successor subprocess whose argv is only (store dir, manifest address, static authority by scene name, the post log) — no pre-death transcript, no starts, no actor count — and its continuation witness equals the never-died timeline's, twice, bit-identically; the only channel across the death is the disk. THE LAW (through-disk splice equivalence): for a window saved from `pre` and any retained boundary b, resume_from(revive(store), b, post) == glide_cells(start, pre+post)[b:] per actor — over a corpus including H=0 and a resume from a FRACTIONAL wall-stopped pose — and equals `splice.resume_cells` from the same pose. THE ROLLBACK AFTER DEATH: authority diverging from the saved prediction at k resumes auth[k:] from the SAVED boundary-k state and lands on glide_cells(auth)[k:] exactly, including k at a fractional pose; k = 0 RESTARTS from the retained cell-aligned start — a red-first find: glide's integer-cell entry MINTS the seed facing from the first command, so a boundary-0 resume carries the predicted facing and the falsifier caught the seed-pose mismatch; the resolution mirrors `cpredict`'s established k == 0 full-re-glide law rather than bending the resume law (`restart_from`, refusing a fractional boundary-0 pose rather than flooring it). THE REFUSES, three voices each owning its law: `RESURRECT-REFUSE` for a correction older than the oldest retained boundary (the durable horizon) and for an INTEGRAL-BUT-INCONSISTENT window — ground contradicting the floor-sampled authority (derived data is checkable data), non-cardinal facing, off-grid pose, negative coordinate; `PERSIST-REFUSE` for store corruption (a flipped byte, a truncated manifest); `SPLICE-REFUSE` for the resume domain. Integrity != truth (L11) is honored on both sides: what is checkable against the live authority IS checked; window PROVENANCE (a tamper derived data cannot see) is stated as `authinput`/`fraud` territory, not silently claimed. MEMBRANE PURITY measured: revive+resume write nothing (the store is byte-identical after), the field is untouched, a second revival is bit-identical. Three pinned scenes: phoenix (3 actors, depth-4 window, die, resume the post log → ADMIT, witness-bound), deep_heal (authority diverges at k=2, the revived process converges → ADMIT), beyond_window (k=3 against a window retaining 4..6 → RESURRECT-REFUSE). Gate stage `resurrect` (4 rows: scenes / death / law / refuse; the death row runs the successor subprocess twice) + red-first `tests/test_resurrect.py` (9 falsifiers, each proven to bite via planted defects — a gutted consistency law, a FLOORED resume pose, a dropped actor across the death, a nearest-boundary fallback all redden; a constant witness moves the pinned scene digests, so the shared-witness channel is guarded by the golden pin). Unit falsifiers 888 → 897; rows 543 → 547; NO placement change. GRADE: the through-death equality, the resume law and its splice tie, the rollback-after-death convergence, the durable-horizon refuse and retained-count tie, the consistency refuses, membrane purity, and determinism are MEASURED (exact, reproducible, a defect diverges); the recovery POINT (revive from the last COMPLETED save — RPO = checkpoint cadence) is a DECLARED operational property inherited from persist's crash-atomicity boundary. `does_not_show`: a crash DURING save (torn writes are DETECTED on load by persist, never prevented); window PROVENANCE (signatures and witness chains — `authinput`/`fraud`); WALL-CLOCK of recovery (`bench.py`, MEASURED-on-named-host); CONCURRENT revivals racing one store; and the netcode-layer analog (N2 `rollback`'s in-memory snapshots — unifying its window with this durable one is a future rung, stated not begun). Stage H now closes end-to-end: the window `horizon` promised, `storecost` priced, `persist` made durable, and `resurrect` makes RECOVERABLE — the law `chunkstate` will distribute.

**Regional state cut (URDRCHS1) — T3.39, MMO Stage I — MEASURED (same-witness reunification checked byte-for-byte against the monolithic persist window).** `tools/terrain/chunkstate.py` + gate stage `chunkstate`: the D16 same-witness law applied to DYNAMIC state — the second Stage-I rung, distributing the recovery law `resurrect` certified. `chunkload` decomposed the STATIC field; the actor state stayed monolithic (one `persist` window holding every actor). chunkstate certifies the decomposition: the world snapshot at a boundary PARTITIONED per region (each actor claimed by the region its pose FLOORS into, on `chunkload`'s own frozen grid; identity rides as the GLOBAL index), each occupied region one content-addressed record — region_record(kx, ky, boundary, entries) = MAGIC | kx | ky | boundary | count | (idx | fx | fy | ground | facing)* | SHA-256, region_record_bytes(m) = 60 + 29·m a CLOSED FORM checked EQUAL to real records, the one digest integrity check + content address + filename — bound by a cut manifest (67 + 40·R, row-major, structure re-checked). THE SAME-WITNESS LAW (the theorem): reunify_cut(cut(state)) == state at EVERY boundary, and the persist records AND the persist MANIFEST built from the reunified window are BYTE-IDENTICAL to the monolith's — identical content addresses end-to-end. Partition, checkpoint regionally, reunify: same witness — the D16 pattern (partition → reunify → monolith witness) now holding for dynamic state, over a corpus including a seam-crossing migrant and a FRACTIONAL wall-stopped pose, at C=8 and C=16. THE CONSISTENT CUT IS FREE: Chandy-Lamport spends its machinery aligning barriers; URDR's lockstep tick already IS a globally synchronous barrier, so every region snapshots the same boundary by construction — a MIXED-BOUNDARY cut is refused, not aligned. MIGRATION is per-boundary re-partition, not state mutation: the pinned seam_walker scene has actor 0 claimed by region (0,0) at boundary 0 and (1,0) at boundary 4, both cuts reunifying exactly. THE AUTHORITY REFUSES (the anti-cheat shape of a distributed snapshot), each typed `CHUNKSTATE-REFUSE` and named: a DOUBLY-CLAIMED actor (one index in two regions — the double-authority exploit), a LOST actor (an index no region claims — no silent despawn), a FOREIGN claim (a pose flooring outside the claiming region — no annexation), a MIXED-BOUNDARY cut; plus the record-level refuses (EVERY single-byte flip and truncation of a pinned record, disordered/duplicate indices, non-cardinal facing in a crafted-but-digested record). THE REGIONAL RESUME: one region revives independently — resume_region preserves identity and equals the global resume law and (by `resurrect`'s certified composition) the never-died suffix per actor. THE ENVELOPE: cut_bytes = Σ region_record_bytes(m_r) + the manifest, checked EQUAL to real bytes, gated by `storecost.within_storage_budget`. Three pinned scenes: consistent_cut (3 actors, 3 regions, reunify == state == monolith record → ADMIT), seam_walker (the migrant, both cuts exact → ADMIT), double_claim (the forged double ownership → CHUNKSTATE-REFUSE). Gate stage `chunkstate` (4 rows: scenes / reunify / region / refuse) + red-first `tests/test_chunkstate.py` (9 falsifiers, each proven to bite via planted defects — a gutted authority law, a region-ordered reunify that loses identity, a lax integrity check, a by-index partition, and an off-by-one envelope all redden). Unit falsifiers 897 → 906; rows 547 → 551; NO placement change. GRADE: the record closed form and round-trip, the exhaustive corruption refuse, the same-witness reunification (state + records + manifest), the migration law, the four authority refuses, the regional-resume equality, the envelope, the budget refuse, and determinism/purity are MEASURED (exact, reproducible, a defect diverges); the REGION GRID choice stays DECLARED (identity and equality hold for every frozen C — which C a deployment picks is sizing policy), and WHICH region SIMULATES which actor between cuts is DECLARED out of scope — this certifies the SNAPSHOT decomposition, not distributed execution. `does_not_show`: cross-machine transport and clocks (a data law, not a network protocol); DYNAMIC REPARTITIONING mid-window (C fixed per cut — the D5 open falsification attempt, not begun); a per-region DEATH boundary (inherited by composition — regional records ride persist's verified restore path; the subprocess falsifier stays resurrect's); live world edits; and cross-placement (Python reference only). The distributed-snapshot story now stands where the streaming story stood: a world you cut into regions is byte-identical to the world you didn't — and the placement batch for the LAT/CHK/CHS family is now due.

**Storage family cross-placed — latstore_rs (URDRLAT4 + URDRLAT5), the placement-batch opener — RE-VERIFIED LIVE by the gate.** `tools/terrain/latstore_rs/latstore.rs`: one independent std-only Rust file (own hand-rolled SHA-256, no crates, no cargo, no shared code with the Python reference) reproducing the SIX pinned storage-family digests against the LIVE conformance goldens, following the `heightfield_rs` live-recompile discipline — the gate compiles it fresh every run and compares to `conformance_storecost.txt` / `conformance_persist.txt`, so a re-pinned Python canon FORCES the port to keep up or `latstore-placement` reddens. WHAT IS PLACED IN FULL: `storecost` (URDRLAT4) — the canonical fixed-width snapshot encoding (4 + 25·n, big-endian, signed int64 poses + facing byte), serialize/deserialize round-trip including negative grounds and near-int64 extremes, window_storage, the budget verdicts, and all three scene digests (pure closed forms — no simulation needed); `persist` (URDRLAT5) — the record law (MAGIC | boundary | payload | SHA-256, 52 + 25·n), the manifest law (47 + 40·(H+1), consecutive boundaries), durable_window_bytes and the REALIZATION IDENTITY (durable == window_storage + envelope_overhead, re-derived independently and checked against real bytes), digest-verify-or-refuse restore, and all three scene digests including the tampered-flip refuse. The binary self-checks the byte laws against REAL bytes each run — closed forms vs real lengths, round-trips, and the EXHAUSTIVE single-byte corruption + truncation sweep over the pinned 77-byte record (every one of 77 flips and 77 truncations must refuse) — printing `selfcheck OK` the gate asserts. THE HONEST SEAM, stated not hidden: the persist scene STATES are derived by a SCENE-DOMAIN fold (the pinned corpus is flat-field eastward walks — no walls, no seams, no gait/subdivision variety), so the GENERAL glide fold is NOT cross-placed by this file; a glide_rs port remains the keystone placement the terrain arc still lacks, and `chunkload` / `chunkstate` / `resurrect` (whose scenes ride real island/mountains trajectories) stay Python-reference-only until it lands. Non-vacuity: a mutated pose layout (POSE_BYTES 25 → 24) MUST diverge from the goldens (`latstore-placement-selftest`); rustc-less hosts record both rows SKIPPED (honestly labelled, row count host-stable). Rows 551 → 553; Rust placements 25 → 26 (the filesystem count the gate enforces); falsifiers unchanged at 906. GRADE: MEASURED on this session's Linux host (rustc, compiled live by the gate, twice, byte-identical) — and re-verified live on ANY host with rustc present, the owner's Windows included, every run. The batch is opened, not closed: remaining debt is glide_rs (the keystone), then the CHK/CHS record families and the opcost→clslo arithmetic — the ledger keeps the list until the batch closes.

**The keystone placed — glide_rs (URDRGLIDE1), the general fold cross-placed over real terrain — RE-VERIFIED LIVE.** `tools/terrain/glide_rs/glide.rs`: the placement the whole batch hinged on. `latstore_rs`'s honest seam was that its persist scene states rode a scene-domain fold; glide_rs removes the foundation of that caveat by independently placing the GENERAL mover — command parse (upper = sprint), the frozen gait law, sub = 2^k micro-steps of ONE >> k (exact shifts on the Q32.32 radix), turn-then-advance facing, floor-sampled ground re-read per micro-step, off-grid and rise > MAX_STEP wall stops at the sub-cell boundary — exercised over the REAL heightfield authority (the URDRHF1 generation lifted verbatim from `heightfield_rs`: seeded SHA-256 lattice, Q16 quintic fade with the load-bearing negative-operand floordiv, sqrt-free island falloff), NOT a flat-field shortcut. Reproduces the three pinned URDRGLIDE1 scene digests against the LIVE `conformance_glide.txt` goldens bit-for-bit — glide_stroll, glide_sprint, and glide_wall, the last a genuine sub-cell wall stop on generated mountains — twice per gate run, with in-binary selfchecks: determinism (the fold twice, equal), the FOLD INVARIANT (every micro pose's ground equals the floor-sampled field at fx >> 32 — the invariant `splice`'s resumability stands on), and micro-step exactness (sub steps sum to exactly ONE for every frozen subdivision). Live-recompile discipline as before: a re-pinned Python canon forces the port to keep up or `glide-placement` reddens; non-vacuity by source mutation (SPRINT_GAIT 2 → 1 diverges glide_sprint AND glide_wall); rustc-less hosts record SKIPPED, row count host-stable. Rows 553 → 555; Rust placements 26 → 27; falsifiers unchanged at 906. GRADE: MEASURED on this session's Linux host (rustc, live-compiled by the gate, twice, byte-identical), re-verified live wherever rustc exists. WHAT THIS UNLOCKS, recorded as the batch's remaining list: rewiring latstore_rs's persist scenes onto real folded states (retiring the scene-domain caveat), then the CHK/CHS record families (`chunkload`'s walk_demand and `chunkstate`'s island scenes ride exactly this fold + this heightfield), `resurrect`'s resume law (the fold from an arbitrary pose is one entry-point away), and the opcost→clslo arithmetic. The keystone is in; the rest of the batch is now record-format work, not physics work.

**The streaming and recovery families cross-placed — streamstate_rs (URDRCHK1 + URDRCHS1 + URDRLAT6, and the URDRLAT5 caveat retired) — RE-VERIFIED LIVE.** `tools/terrain/streamstate_rs/streamstate.rs`: the batch's second repayment, one std-only Rust file for the three families that share the fold, the field, and the digest law. PLACED: `chunkload` (URDRCHK1) — chunk records, the field manifest, byte-exact reassembly, DEMAND SETS computed by a recording getter over the general fold, and the equal-or-refuse partial glide (island16 / walk_demand / cold_start reproduce, including the demand-set trajectory-equality witness); `chunkstate` (URDRCHS1) — regional records, the cut manifest, partition-by-floor-cell, and the SAME-WITNESS reunification with the four authority refuses re-implemented (mixed-boundary, annexed, doubly-claimed, lost — the in-binary selfcheck also re-proves reunify == state AND the monolithic persist-record equality at C=8 and C=16); `resurrect` (URDRLAT6) — the resume-from-pose entry (the fold generalized to an arbitrary Q32.32 pose — `splice`'s `_fold_from`, now independently placed), phoenix / deep_heal / beyond_window reproducing with the through-fold witness; AND the URDRLAT5 persist scenes re-derived through the GENERAL fold over the flat field, with an in-binary equality proof (general-fold boundary states == the analytic east-walk states at every boundary) — RETIRING latstore_rs's scene-domain caveat: the persist digests now reproduce from the placed general mover, walls-and-all capable, not an analytic shortcut. In-binary selfchecks each run: reassembly byte-equality at two chunk sizes, the same-witness law, resume == the never-died suffix FROM A FRACTIONAL WALL-STOPPED POSE on real mountains, and per-family corruption refuses. Live-recompile discipline: twelve scenes against the LIVE conformance goldens, compiled fresh twice per gate run, byte-identical; non-vacuity by source mutation (SPRINT_GAIT 2 → 1 diverges walk_demand and every sprint-bearing trajectory); rustc-less hosts record SKIPPED. All twelve digests reproduced on the first successful compile. Rows 555 → 557; Rust placements 27 → 28; falsifiers unchanged at 906. GRADE: MEASURED on this session's Linux host (rustc, live-compiled by the gate, twice, byte-identical), re-verified live wherever rustc exists. THE BATCH LEDGER after this entry: repaid — storecost (full), persist (full, caveat retired), glide (the keystone), chunkload, chunkstate, resurrect. Remaining — the opcost → clslo arithmetic family (micro-counts now derivable from the placed fold; slo/clslo closed forms; priogov's drain queue), one commit from closure.

**THE PLACEMENT BATCH CLOSES — latarith_rs (URDROPC1 + URDROPC2 + URDROPC3 + URDRLAT2 + URDRLAT3), RE-VERIFIED LIVE.** `tools/terrain/latarith_rs/latarith.rs`: the arithmetic family, one std-only Rust file, SEVENTEEN scenes against the LIVE conformance goldens — all reproduced on the first successful compile. PLACED: `opcost` (URDROPC1) — the exact work counts: glide micro-steps via the verbatim-lifted fold (with the wall witness: glide_wall's count 23 strictly below its bound 32 — the bound proven non-vacuous in-binary), warden edge checks (W−1)H + W(H−1), admit sub-steps Σ|dx|+|dy|, wardhom's legal-edge column count (|Δh| ≤ MAX_STEP adjacency law, barrier8's 96 columns), and the per-tick envelope; `govern` (URDROPC2) — the FIFO governor re-implemented (admit a prefix within budget, defer the rest), the three drain schedules reproducing; `priogov` (URDROPC3) — priority with aging (effective priority = base + age·wait, index-stable ordering, prefix-not-skip admission), the three schedules reproducing; `slo` (URDRLAT2) and `clslo` (URDRLAT3) — the closed forms ceil(N/⌊budget/cost⌋) + H and ceil(N≥p/m) + H, with the by-name refuse verdict format (CLSLO-REFUSE:p3) reproduced. THE SOUNDNESS CORE, in-binary every run: the REAL 24-check clslo corpus — read from `verify.py` at build time, not carried in from prose (the phantom "32/345-config" numbers died on contact with the source, which is the reading discipline working) — 4 class configs × 3 budgets × 2 aging rates, the closed-form per-class bound EQUAL to the Rust-native priogov scheduler's last-served tick per class, the interleaved-arrival actor construction ported exactly (the scheduler is the NEUTRAL RULER: it never sees the formula it validates); plus slo's bound == the real FIFO drain length at five budgets, count ≤ bound with the wall witness STRICT, the governor guarantees (never-overrun, progress, ≤ N ticks), and the one-class reduction (clslo's single class == slo's uniform number). Non-vacuity: the defect anchor is the CEIL DIVISION itself (every scene walks lowercase, so a gait anchor would have been vacuous — chosen by reading the corpus, not by habit); mutated ceil → floor both DIVERGES the tiered table digests and BREAKS the in-binary soundness corpus (double catch, verified before wiring). Live-recompile discipline as the family before it; SKIPPED rows without rustc. Rows 557 → 559; Rust placements 28 → 29; falsifiers unchanged at 906. GRADE: MEASURED on this session's Linux host (rustc, live-compiled by the gate, twice, byte-identical), re-verified live wherever rustc exists. THE LEDGER AT ZERO: every rung of the Stage-B..I streak — storecost, persist, glide, chunkload, chunkstate, resurrect, opcost, govern, priogov, horizon (its window law embedded throughout), slo, clslo — now carries an independent std-only Rust placement re-verified LIVE by the gate on every run. The batch opened at nine Python-only rungs and closes at zero, each placement earning its digests against the live goldens with a proven-biting defect anchor. The covenant is discharged: recorded debt became scheduled repayment became closed ledger. What remains outside the batch, honestly: refusal-MESSAGE parity (verdicts and digests are placed; prose is Python's), the C99 axis for these families (the terrain arc's placements are Rust-only, as heightfield_rs before them), and wall-clock everything (`bench.py`, MEASURED-on-named-host, forever off-gate).
