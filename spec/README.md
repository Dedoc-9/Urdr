...
| [`D17-invariant-detectors.md`](D17-invariant-detectors.md) | **The invariant-detector admission law** (a *meta*-contract, not a subsystem) — names the law every gated detector already follows: a detector is `(Dom, Inv, W, ~, R)` — decidable domain, exact invariant, independently-checkable witness, a declared **equivalence relation** `~` (a group is only the invertible special case; monoids/equivalences are general), and total typed REFUSE off-domain — admitted iff membership is decidable, REFUSE is total, the invariant is exact + witnessed + constant on a `~`-corpus, and a non-invariance defect reddens. Validated by **retro-admission** (D14/D15/D16/rigidity/criticality all satisfy it — falsifiable by existing code). "What survives?" as an architecture; the object's **invariant profile** is `{detector → value \| REFUSE}`. |
| [`D19-abductive-gauntlet.md`](D19-abductive-gauntlet.md) | **The abductive-gauntlet contract** (the hypothesis boundary — pipeline **SPECULATIVE**; first rung `W1` **LANDED**). How candidate hypotheses would be admitted for verification: every batch recorded at the līmes (content-addressed), floor grade on entry (*a proposal is provenance, never evidence* — `URDR-EVIDENCE-UNEARNED` extended to machine-generated text), elimination only by admitted D17 detectors with witnessed verdicts (*elimination is measured; survival is not evidence*), verification by the existing gate unchanged, one-way boundary. Its first rung has LANDED: `W1`, the exact winding-number detector with a theorem-backed Loewner-1948 corpus (gate stage `winding`, the 8th D17 detector, cross-placed (`winding_rs`), THEOREMS T23, graded in D5) — while the pipeline itself stays SPECULATIVE: no proposer, no recorded batch, no `ABDUCT-REFUSE` in the tree. |
...
(There is no `D2`/`D3`, and `D18` is unassigned; the D-series is not gap-free by accident —
numbering follows the order laws were written, not a table of contents.)
