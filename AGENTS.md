...
| `tools/terrain/` | `urdr-terrain` T1: the `URDRHF1` deterministic heightfield canon (SHA-seeded lattice noise, Q16 quintic FBM, sqrt-free island falloff; same seed → same bytes on any host) — the D14 procedural modality; ladder + the idle law in `docs/terrain_studio_brief.md` |
