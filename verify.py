...
class Gate:
    ...
    def liveness(self):
        ...
        SHA-256(session | tick) is computable by every observer, so an adversary on the path resets
        the client's counter forever while the server is gone. That is the COUNTERFEIT RESET, closed
        by HMAC over pre-shared material bound to clockauth's server-attested tick. The countdown is
        pure integer subtraction with NO clamp, firing exactly before zero. Rows: scenes, auth,
        descent, selftest."""
        if os.path.join(ROOT, "tools", "terrain") not in sys.path:
            sys.path.insert(0, os.path.join(ROOT, "tools", "terrain"))
        try:
            ...
            self.record("liveness", False, f"import failed (liveness): {exc}")
            return
        try:
            ref_ok = all(LV.scene_result(n) == LV.golden(n) for n in LV.SCENES)
            ref_ok = ref_ok and all(LV.scene_result(n) == LV.scene_result(n) for n in LV.SCENES)
            ref_ok = ref_ok and LV.emitted_matches_pinned()
        except Exception as exc:
            self.record("liveness:scenes", False, f"reference failed: {exc}")
            return
        self.record("liveness:scenes", ref_ok,
                    "auth + descent + plants reproduce URDRLIV1 digests, and the pinned corpus is "
                    "EXACTLY what `liveness.py --emit` produces — so no hex is hand-inscribed, while "
                    "the file stays FROZEN rather than regenerated by this gate, because a golden the "
                    "gate rewrites at run time cannot detect drift (L23: a checker that cannot fail). "
                    "The module emits, a human pins, the gate compares"
                    if ref_ok else "a liveness scene drifted from its digest, or the corpus is not the "
                                   "module's own emitted output")
        auth_ok = True
        try:
            unkeyed, keyed, attempts = LV.forgery_census()
            auth_ok = (unkeyed, keyed, attempts) == (12, 0, 12) and attempts > 0
            auth_ok = auth_ok and LV.keyed_auth_closes_the_counterfeit_reset()
            auth_ok = auth_ok and LV.replay_window() == (1, 12)
            (lo, hi), inside, outside = LV.bound_to_clockauth_band()
            auth_ok = auth_ok and inside == hi - lo + 1 and outside > 0 and lo <= hi
            good = LV.token(LV.SECRET, LV.SESSION, 5)
            auth_ok = auth_ok and LV.verify_token(LV.SECRET, LV.SESSION, 5, good)
            auth_ok = auth_ok and not LV.verify_token(b"wrong", LV.SESSION, 5, good)
            auth_ok = auth_ok and not LV.verify_token(LV.SECRET, LV.SESSION, 6, good)
            auth_ok = auth_ok and not any(
                LV.verify_token(LV.SECRET, LV.SESSION, 1, j)
                for j in (None, "", 0, b"", b"\x00" * LV.TOKEN_BYTES))
        except Exception:
            auth_ok = False
        self.record("liveness-auth", auth_ok,
                    "the heartbeat proves POSSESSION rather than recomputability: an unkeyed token is "
                    "a hash of PUBLIC identifiers and is forged 12 of 12 by any observer, where the "
                    "HMAC over pre-shared material is forged 0 of 12 — that gap IS the counterfeit "
                    "reset closed, and both denominators are reported so neither number can read as "
                    "an empty search; the token is bound to clockauth's SERVER-ATTESTED tick rather "
                    "than to wall-clock, every tick in the admissible band authenticating and a token "
                    "from one tick serving no other; and REPLAY IS BOUNDED TO EXACTLY ONE TICK rather "
                    "than eliminated, which is the honest form — an intercepted token is worth only "
                    "the tick it was already evidence for, so it cannot mask a stall, and claiming "
                    "replay was PREVENTED would be the inflation this repo refuses"
                    if auth_ok else "the liveness auth law did not hold")
        desc_ok = True
        try:
            survived, raised, patience = LV.well_founded_descent()
            desc_ok = raised and survived == patience - 1 and (survived, patience) == (3, 4)
            mn, allpos = LV.budget_never_leaves_the_naturals()
            desc_ok = desc_ok and mn == 1 and allpos
            hs, hr = LV.heartbeat_resets_the_budget()
            desc_ok = desc_ok and not hr and hs == LV.PATIENCE * 3
            desc_ok = desc_ok and LV.step_does_not_swallow()
            for bad in (0, -3, "4", None):
                try:
                    LV.step(bad, None)
                    desc_ok = False
                except LV.LivenessError:
                    pass
        except Exception:
            desc_ok = False
        self.record("liveness-descent", desc_ok,
                    "the countdown is PURE INTEGER SUBTRACTION over the naturals with no defensive "
                    "clamp, and the fault fires exactly when the next value would be 0 — so the "
                    "returned budget is never 0 and never negative, the minimum ever seen is 1, and "
                    "termination is a WELL-FOUNDED relation on (N, <) rather than a loop with a "
                    "floor: from a full budget under total silence exactly PATIENCE-1 = 3 ticks "
                    "survive and the 4th raises; an authenticated heartbeat restores the full budget "
                    "so an alive server is never declared offline, which is the validity half a "
                    "watchdog that always fires would fail; and the fault is let OUT of step rather "
                    "than swallowed"
                    if desc_ok else "the liveness descent law did not hold")
        red_ok = False
        try:
            clamped, honest = LV.clamp_plant_never_fires()
            red_ok = clamped and honest == LV.PATIENCE - 1
            red_ok = red_ok and LV.sliding_window_plant_masks_a_stall() == (4, 8, 40, 40)
            red_ok = red_ok and LV.masking_ladder_is_strict()
            sw, hs2 = LV.swallowing_plant_hides_the_fault()
            red_ok = red_ok and sw and hs2 == LV.PATIENCE - 1
            red_ok = red_ok and LV.baseexception_would_abort_the_gate() == (True, False)
            red_ok = red_ok and issubclass(LV.ServerOffline, Exception)
        except Exception:
            red_ok = False
        self.record("liveness-selftest", red_ok,
                    "four plants bite and one of them settles a design question with data. The "
                    "'defensive' max(0, budget-1) CLAMP runs 500 ticks of total silence without ever "
                    "firing, so the line that looks like hardening is the liveness residual reopened. "
                    "The MASKING LADDER measures what each relaxation buys an adversary holding ONE "
                    "intercepted token through a 40-tick stall: the honest step hides 4 ticks, a "
                    "bounded sliding window hides 8, and a verifier that accepts ANY HISTORICAL token "
                    "hides all 40 — that third one is the implementation that ships in the wild, "
                    "asking 'is this a token we recognise' instead of 'is this the token now demands', "
                    "authenticating possession and forgetting freshness entirely. A SWALLOWING wrapper "
                    "that catches its own fault hides it forever, which is the failure mode a "
                    "BaseException base class was proposed to prevent — and that proposal was MEASURED "
                    "rather than argued: this gate wraps every stage in `except Exception`, so a "
                    "normal Exception is RECORDED as a red row and the run stays byte-identical while "
                    "a BaseException subclass ESCAPES and aborts the process, leaving no row, no "
                    "remaining stages, no GATE FAILED line and nothing to diff. (True, False), pinned. "
                    "The anti-swallowing guarantee is therefore a plant plus an assertion, which is "
