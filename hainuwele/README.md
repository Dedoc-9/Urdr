...
| [`worldgeom.py`](../tools/terrain/worldgeom.py) | `URDRWGM1` | A castle generated from what it IS — authored parts, derived prisms, integer octagons, and the SUPPORT law (founded, carried, or a DECLARED overhang) that a merlon and a machicolation both forced | `worldgeom` | [test](../tests/test_worldgeom.py) | [conf](../tools/terrain/conformance_worldgeom.txt) | [brief](../docs/worldgeom_brief.md) |
| [`versionarc.py`](../tools/terrain/versionarc.py) | `URDRVRA1` | A version that stamps evidence must be documented — the ungated wall-clock class's first door, gating the ARTIFACT'S EVIDENCE rather than the artifact, with token boundaries so `v1.14` cannot pass as `v1.1` | `versionarc` | [test](../tests/test_versionarc.py) | [conf](../tools/terrain/conformance_versionarc.txt) | [brief](../docs/versionarc_brief.md) |
| [`admit.py`](../tools/terrain/admit.py) | `URDRADM1` | The instrument reports and the gate ADJUDICATES — a truncated replay printed a record shaped exactly like a finished one, so every run now emits its class, its trace and workload identities and a status that is the CONJUNCTION of its class's conditions, and this reader recomputes them (DISAGREEMENT is its own verdict) | `admit` | [test](../tests/test_admit.py) | [conf](../tools/terrain/conformance_admit.txt) | [brief](../docs/admit_brief.md) |
| [`castlecost.py`](../tools/terrain/castlecost.py) | `URDRCCS1` | The castle's price and the SEPARATION that outlives it — halving the terrain reach left the castle's cost where it was (±8.2%), so world reach and authored-geometry fill are independent axes; the digest chain used as a COST ORACLE bounds what the feature costs to HAVE against what it costs to SHOW | `castlecost` | [test](../tests/test_castlecost.py) | [conf](../tools/terrain/conformance_castlecost.txt) | [brief](../docs/castlecost_brief.md) |
| [`fibre.py`](../tools/terrain/fibre.py) | `URDRFBR1` | The object beside the digest, and ONE claim made of it — a Linux replay reproduces every Windows checkpoint, and the census it enables shows the equivalence is the FIBRES OF THE RENDER MAP rather than a coordinate projection: forgetting the clip phase is refuted by the corpus | `fibre` | [test](../tests/test_fibre.py) | [conf](../tools/terrain/conformance_fibre.txt) | [brief](../docs/fibre_brief.md) |
| [`armpair.py`](../tools/terrain/armpair.py) | `URDRARM1` | The equality OUTLIVES the code that proved it — both retired raster arms keep their replay equalities as comparisons of committed bytes, across TWO generations: generation 2's arm identity is DERIVED from the record's own raster stamp rather than declared, and the two generations' shared rasteriser MEASURES the session drift that keeps the compound figure UNDERDETERMINED | `armpair` | [test](../tests/test_armpair.py) | [conf](../tools/terrain/conformance_armpair.txt) | [brief](../docs/armpair_brief.md) |
| [`voxref.py`](../tools/terrain/voxref.py) | `URDRVXF1` | RUNG ZERO of the voxel arc — the observable frozen BEFORE any reduction exists: O_t = (colour digest, depth digest), coverage a top-left PARTITION rather than the castle's cover, and draw order proved observationally irrelevant on a corpus checked to contain the coincident-face case the partition alone does not close | `voxref` | [test](../tests/test_voxref.py) | [conf](../tools/terrain/conformance_voxref.txt) | [brief](../docs/voxref_brief.md) |
| [`voxcoarse.py`](../tools/terrain/voxcoarse.py) | `URDRVXC1` | How coarse the frozen observable actually is, measured BEFORE any reduction leans on it — 1728 declared states over a committed record the gate re-derives from and re-renders a sample of; the largest fibre is the EMPTY VIEW and is characterised rather than counted, and over the 603 states that see something the observable separates 557 | `voxcoarse` | [test](../tests/test_voxcoarse.py) | [conf](../tools/terrain/conformance_voxcoarse.txt) | [brief](../docs/voxcoarse_brief.md) |
| [`voxray.py`](../tools/terrain/voxray.py) | `URDRVXR1` | THE GEOMETRIC ORACLE, shipped REPORTED and not certifying — exact integer voxel traversal answering which voxel and face a camera ray enters first, sharing only the scene and an AUDITED camera contract with the renderer; it names a THIRD answer at the pixel where face culling and the reference disagreed, and measures the reference at 17.4% correspondence as committed against 87.0% with the six windings reversed | `voxray` | [test](../tests/test_voxray.py) | [conf](../tools/terrain/conformance_voxray.txt) | [brief](../docs/voxray_brief.md) |
| [`voxmicro.py`](../tools/terrain/voxmicro.py) | `URDRVXM1` | THE ORACLE QUALIFIED AND THE RESIDUE DECOMPOSED — twenty-four elementary scenes with fifty-six expectations written before they ran, the eye-inside-matter semantics decided against a direction-blindness law rather than a preference, the measured <=1px sampling offset subtracted BY NAME so every remaining fate is a lower bound on real defect, and 2040 impossible interior-face pixels surviving the winding reversal: a defect measured without the oracle and without any sampling caveat | `voxmicro` | [test](../tests/test_voxmicro.py) | [conf](../tools/terrain/conformance_voxmicro.txt) | [brief](../docs/voxmicro_brief.md) |
| [`voxevent.py`](../tools/terrain/voxevent.py) | `URDRVXE1` | DOES THE VISIBLE SURFACE GROW WITH THE GEOMETRY OR WITH THE INCIDENCES — a subdivision ladder proved to move no world point, multiplying primitives 512x while merged visible regions move 3% at the rung where the sampler is unstressed; the far end is CENSORED BY THE RAY BUDGET and says so in its own row; four incidence-degenerate families with counts predicted before they ran; and the simultaneous-crossing convention `voxray` never declared, which decides which cell is REACHED rather than which face is named | `voxevent` | [test](../tests/test_voxevent.py) | [conf](../tools/terrain/conformance_voxevent.txt) | [brief](../docs/voxevent_brief.md) |
| [`voxcand.py`](../tools/terrain/voxcand.py) | `URDRVXD1` | A REPAIR CANDIDATE THAT IS NOT THE REFERENCE — two fixes isolated as a 2x2 that refuted its own motivating prediction (the weight fix is HARMFUL applied alone to the wrong winding), impossible pixels 14032 -> 661 and oracle agreement 15.2% -> 91.4%, perspective correction refused on evidence, `voxref` asserted untouched so the frozen census stays valid, and fact three asserted RED so the census may not be regenerated | `voxcand` | [test](../tests/test_voxcand.py) | [conf](../tools/terrain/conformance_voxcand.txt) | [brief](../docs/voxcand_brief.md) |
| [`voxtie.py`](../tools/terrain/voxtie.py) | `URDRVXT1` | A CARVE-OUT PROPOSED AND REFUSED BY MEASUREMENT — every residual pixel classified by one-sided screen AND viewpoint limits into stable / boundary / degenerate, giving 378 / 198 / 561 over the declared trace and 80 stable against 1 boundary among the impossible-face pixels; two further defects isolated (the projection's axes round opposite ways, and vertices quantise to whole pixels) and neither applied; four tie orderings measured against a CEILING of 1 of 13 with none adopted | `voxtie` | [test](../tests/test_voxtie.py) | [conf](../tools/terrain/conformance_voxtie.txt) | [brief](../docs/voxtie_brief.md) |
| [`voxfate.py`](../tools/terrain/voxfate.py) | `URDRVXS1` | CONDITION THE POPULATION, THEN ASK WHAT BROKE IT — every earlier fate distribution in this arc mixed real defects with event-surface samples and edge/corner degeneracies, and a mechanism read off that mixture is a mechanism for an average of three unrelated things; conditioned on `voxtie`'s 378 STABLE pixels the answer does not split (not_covered 318 / depth_rejected 58 / phantom 2, and 78 of the 80 impossible-face pixels not_covered), the contamination is DEMONSTRATED by re-running with the sampling branch enabled (269 absorbed, signal gone), and the minimal counterexample is frame 4 pixel (10, 0) — the pixel this whole arc started from | `voxfate` | [test](../tests/test_voxfate.py) | [conf](../tools/terrain/conformance_voxfate.txt) | [brief](../docs/voxfate_brief.md) |
| [`voxfill.py`](../tools/terrain/voxfill.py) | `URDRVXL1` | THE FILL RULE ONE VARIABLE AT A TIME, AND THE ARM THAT WON WAS NOT ON THE LIST — every coverage rejection classified algebraically before any arm runs (215 bias_only / 100 outside, EVERY one by under a pixel once a published ranking defect was found and corrected / 3 bbox), the bounding box eliminated by a check proved non-vacuous rather than by an empty class, and the deciding 2x2 varying the ORACLE'S convention as well as the rasteriser's: both consistent pairings beat both mixed ones, impossible faces fall 152 -> 4 on the sample point ALONE with no oracle involved, and the rung's own leading hypothesis is refuted by its own control | `voxfill` | [test](../tests/test_voxfill.py) | [conf](../tools/terrain/conformance_voxfill.txt) | [brief](../docs/voxfill_brief.md) |
| [`voxconv.py`](../tools/terrain/voxconv.py) | `URDRVXN1` | THE POPULATION DERIVED TWICE, BECAUSE MOST OF IT WAS THE CONVENTION — 1137 disagreeing pixels at the corner sample become 104 at the centre, and the DEGENERATE class that excused 561 of them from ever being called defects collapses to 10, because integer screen coordinates are precisely the rays that land on lattice-plane crossings; the coverage diagnosis SURVIVES with its share intact (84.1% -> 83.1%) while the ownership class VANISHES (bias_only 215 -> 0), exonerating the top-left rule a second time by a route `voxfill` did not take; and what is left is one untested mechanism — the floored projected VERTEX, a defect in the projection rather than the fill | `voxconv` | [test](../tests/test_voxconv.py) | [conf](../tools/terrain/conformance_voxconv.txt) | [brief](../docs/voxconv_brief.md) |
| [`voxgrid.py`](../tools/terrain/voxgrid.py) | `URDRVXG1` | IS THE DEGENERACY THE LATTICE OR THE SAMPLING GRID — both, and the subdivision ladder separates them: the artefact share is 96% at the base lattice (1017 crossings become 40) and 50% at scale 8 (11119 become 5507), because subdividing by s multiplies the plane density by s, so `voxconv`'s published claim that the rate is a fact about the sampling grid carried NO SCALE and is corrected here; and `voxevent`'s actual conclusions SURVIVE — visible faces and merged regions move under 3%, and the ray-budget censoring is exact under both conventions at two different totals | `voxgrid` | [test](../tests/test_voxgrid.py) | [conf](../tools/terrain/conformance_voxgrid.txt) | [brief](../docs/voxgrid_brief.md) |
| [`voxslack.py`](../tools/terrain/voxslack.py) | `URDRVXK1` | HOW FAR IS EACH WRONG PIXEL FROM THE LAW THAT DECIDED IT — diagnostic only, with a DECLARED sign convention and exact integer slacks in the reference's own units: every coverage miss is within a pixel of the surface and none beyond (215 exactly on it at slack -1, the top-left bias; 103 sub-pixel, the floored vertex), and THE 58 DEPTH REJECTIONS ARE NOT A MARGIN DEFECT AT ALL — 46 lose by a whole cell or more, none should have won, and their coverage slack is hugely positive, so the fate split is really 376 coverage and 2 anomaly with the 58 counted at the wrong end | `voxslack` | [test](../tests/test_voxslack.py) | [conf](../tools/terrain/conformance_voxslack.txt) | [brief](../docs/voxslack_brief.md) |
| [`voxwin.py`](../tools/terrain/voxwin.py) | `URDRVXW1` | CHASE THE WINNER AND THE DECOMPOSITION CLOSES — at 56 of the 58 redirected pixels the ray geometrically MISSES the face the rasteriser awarded (exact integer ray/plane arithmetic, no float and no epsilon, planted in every direction) and the winner is the oracle's own answer one pixel over at every one; the other 2 meet both faces at equal depth and are PRECISELY `voxslack`'s two exact-depth ties, asserted as SET EQUALITY rather than a matching count, which merges `voxtie`'s parked question into a population of two — leaving 374 coverage / 2 tie / 2 phantom against `voxfate`'s 318/58/2 | `voxwin` | [test](../tests/test_voxwin.py) | [conf](../tools/terrain/conformance_voxwin.txt) | [brief](../docs/voxwin_brief.md) |
| [`voxproj.py`](../tools/terrain/voxproj.py) | `URDRVXP1` | THE CANDIDATE LAW, PREDICTED BEFORE IT RAN AND REFUSED ON EVIDENCE — one variable (the projected vertex FLOORED in the control, ROUNDED TO NEAREST in the candidate, checked to differ by at most one sub-pixel unit and to agree where the division is exact), with five predictions pinned as DATA before a frame was rendered and EVERY ONE scored: 2 hit, 3 missed, and net agreement falls eleven pixels so the candidate is refused; 91 of the 103 sub-pixel misses survive round-to-nearest, which ELIMINATES the rounding direction as the mechanism by measurement rather than argument | `voxproj` | [test](../tests/test_voxproj.py) | [conf](../tools/terrain/conformance_voxproj.txt) | [brief](../docs/voxproj_brief.md) |
| [`voxsample.py`](../tools/terrain/voxsample.py) | `URDRVXA1` | ARE THE RASTERISER AND THE ORACLE TALKING ABOUT THE SAME SAMPLE POINT — the integer camera basis is exactly orthonormal on only 5 of 8 frames so `ray_for_pixel` is an approximate inverse on the rest (bounded, as an exact fraction, under a quarter of a sub-pixel unit); at FULL precision the oracle's face contains the sample at 316 of 318 and the winner's excludes it at 56 of 56, SET-EQUAL to `voxwin`'s world-space test; and putting back the one `>> 16` in `_project` pushes 91 pixels OUT of a face that contains them and pulls 53 IN to one that does not — the seam is upstream of every rung this arc has run, and the 215 on-edge pixels are made by that truncation rather than by the top-left rule | `voxsample` | [test](../tests/test_voxsample.py) | [conf](../tools/terrain/conformance_voxsample.txt) | [brief](../docs/voxsample_brief.md) |
| [`voxcam.py`](../tools/terrain/voxcam.py) | `URDRVXB1` | THE CANDIDATE THAT WORKS, AND IT IS STILL NOT THE REFERENCE — one variable (the `>> 16` `voxref._project` performs, TRUNCATING in the control and carried at FULL PRECISION in the candidate, with the forced near-plane re-expression PROVED to name the same plane for every integer), five predictions pinned as DATA before a frame was rendered and every one scored: 4 hit, 1 missed, and agreement goes 45550 -> 45889 with impossible faces 152 -> 70 and +357 gained against -18 lost, where `voxproj` gained 23 and lost 34; the miss is that both exact ties closed, but what closed is the DEPTH tie and not the geometry, so `voxwin`'s world-space edge crossing stands; 21 of the 378 survive, 18 new pixels break, and NOTHING IS PROMOTED because full precision scales every interpolated depth by 2^16 and moves the DEPTH half of the observable — a contract change, not a bug fix | `voxcam` | [test](../tests/test_voxcam.py) | [conf](../tools/terrain/conformance_voxcam.txt) | [brief](../docs/voxcam_brief.md) |
| [`voxwork.py`](../tools/terrain/voxwork.py) | `URDRVXO1` | THE PERFORMANCE ARC'S FIRST RUNG, AND IT OPTIMISES NOTHING — a ruler, PROVED INERT before any arm exists (the seventh transcription of the committed loop, its buffers required to equal `voxref.render`'s AS LISTS on every frame, not as digests). 664553 pixels walked for 55296 output pixels, TWELVE TIMES OVERDRAW, and the fate of a walked pixel is a PARTITION asserted per frame: 63% outside the triangle, 27% covered and beaten on depth, 10% written — two losses of DIFFERENT KINDS, kept apart because coverage is answerable per tile and depth per primitive. Model == execution twice (the walk derived from bounding boxes without running the inner loop; 6633411 multiplies from seven declared coefficients), one law reddened before shipping and exposed that THE SETUP IS A FIFTH OF THE ARITHMETIC, and NO WALL CLOCK may enter — enforced from this module's own AST | `voxwork` | [test](../tests/test_voxwork.py) | [conf](../tools/terrain/conformance_voxwork.txt) | [brief](../docs/voxwork_brief.md) |
| [`voxsilo.py`](../tools/terrain/voxsilo.py) | `URDRVXH1` | THREE SILOS, EIGHT CELLS, AND THE FULL COMBINATION IS NOT THE BEST ONE — deliberate functional siloing (Ensor 1988 inverted) with a contract no silo may negotiate: byte-identical colour and depth AS LISTS in all eight cells. THE CONTRACT CAUGHT AN UNSOUND OPTIMISATION ON ITS FIRST RUN: the convex-combination premise every hierarchical-Z scheme rests on is FALSE here at 215300 of 245183 covered pixels, because the top-left bias makes the weights sum to `area + B` while the depth still divides by `area` — repaired by `zmin + (zmin*B)//area`, with the naive cull kept RUNNABLE as a plant that must still move the observable. GA is cheapest in multiplies and GTA is DEARER: arm T spends 269418 multiplies to retire 42101 pixels that arm A has already made free, and omega(T,A) = 810354/139476 is ABOVE ONE — not redundancy but SUBTRACTION. The mean walked bounding box is 56 against a tile of 64 | `voxsilo` | [test](../tests/test_voxsilo.py) | [conf](../tools/terrain/conformance_voxsilo.txt) | [brief](../docs/voxsilo_brief.md) |
| [`voxpath.py`](../tools/terrain/voxpath.py) | `URDRVXJ1` | A SECOND DECLARED TRACE — 31 frames, eight named episodes, ONE CONTINUOUS WALK down the longest corridor in this world with the eye never in matter and one declared discontinuity. AND THE HEADLINE INVERTS THE ARC'S PREMISE: at 1/32 of a voxel per frame the COLOUR buffer is unchanged at 6900 of 6912 pixels and THE EXACT OBSERVABLE AT ONLY 811, because depth is a continuous function of camera position and `O_t` contains it exactly — there is NO temporal sparsity in the observable to exploit at any speed, so the certificate must be about OWNERSHIP with depth RECONSTRUCTED. Also finds that the REVERSED winding collapses a declared case (8 distinct observables committed, 7 reversed), and ships the next rung's PRE-REGISTRATION one commit before any arm exists | `voxpath` | [test](../tests/test_voxpath.py) | [conf](../tools/terrain/conformance_voxpath.txt) | [brief](../docs/voxpath_brief.md) |
| [`voxcond.py`](../tools/terrain/voxcond.py) | `URDRVXQ1` | FIVE CONDITIONAL CERTIFICATES SCORED AGAINST A PREDICTION COMMITTED ONE COMMIT EARLIER, quoted by digest rather than restated. 3 hit, 2 MISSED — and the misses are the result: an ownership certificate VERIFIES a remembered owner rather than DETERMINING one, so it is far cheaper than predicted. Every cheap camera-side predicate is UNSOUND for one reason (the camera moved; depth is continuous in camera position and `O_t` contains it exactly), each refutation kept RUNNABLE. P4 retires 4038404 operations for 210871 — nineteen times — BUT the tiled loop it sits on costs 42913656 against the reference's 23201850, so the arrangement retires NOTHING: mechanism established, implementation not competitive, with a law so the first is never quoted without the second | `voxcond` | [test](../tests/test_voxcond.py) | [conf](../tools/terrain/conformance_voxcond.txt) | [brief](../docs/voxcond_brief.md) |
| [`voxstate.py`](../tools/terrain/voxstate.py) | `URDRVXU1` | A STATE LATTICE AND FOUR WAYS TO WALK IT, WITH NO CERTIFICATE BUILT — 16 camera states on the only two axes this world admits (mutating geometry for a third would invalidate every frozen record), four traversals differing ONLY in inheritance and all required to be permutations of one state set. The measurement killed the rung's first law: A QUARTER OF A VOXEL SATURATES THE OBSERVABLE (4241-6472 of 6912 pixels differ across EVERY adjacent pair), so there is no step size short of zero at which distance discriminates — which means distance CANNOT CONFOUND the next rung's adjacent-vs-non-adjacent comparison, and the traversals differ only in STRUCTURE. Ships voxmanifold's pre-registration one commit before any traversal runs | `voxstate` | [test](../tests/test_voxstate.py) | [conf](../tools/terrain/conformance_voxstate.txt) | [brief](../docs/voxstate_brief.md) |
| [`voxmanifold.py`](../tools/terrain/voxmanifold.py) | `URDRVXV1` | DOES CERTIFICATE VALIDITY HAVE STRUCTURE — YES ON LOCALITY, NO ON THE MANIFOLD. Scored against a prediction committed one commit earlier and quoted by digest; 2 of 5 hit. SURVIVES: retirement rises monotonically with inheritance quality (Z1 1713400, Z2 2793500, Z3 3360592), so a nearer predecessor retires nearly twice the scan order's. DIES THREE WAYS: adjacency gives only a 28% per-pair edge, not a categorical one; ALL 16 of 16 states are validity BOUNDARIES so there is no cheap interior; and 0 of 24 sub-additivity triples are negative, so an intermediate state never contributes reusable structure. My own M4 was ambiguously worded and the LITERAL text was scored (MISS); the best traversal is still 1.56x the reference | `voxmanifold` | [test](../tests/test_voxmanifold.py) | [conf](../tools/terrain/conformance_voxmanifold.txt) | [brief](../docs/voxmanifold_brief.md) |
| [`voxfriction.py`](../tools/terrain/voxfriction.py) | `URDRVXY1` | BENEFICIAL FRICTION — can a probe cheaper than the work it avoids tell when to bother? YES, AND THE TRANSITION IS SHARP: by distinct owners the payoff runs +3067498 / +123524 / +125408 at 1-3 owners and turns NEGATIVE at 4, where THE CERTIFICATE STOPS FIRING ENTIRELY across 245 tiles; single-owner tiles carry 93% of all value. The probe is COST-SHARED not cost-compared — collecting the owner set IS the certificate's own first step — and the payoff is a COUNTERFACTUAL that was RUN, every tile rastered twice, which makes this a DIAGNOSTIC and not an implementation. The correctness asymmetry is the contract: a wrong probe decision either way costs performance and NEVER `O_t`, proved on both degenerate limits. NO threshold is declared | `voxfriction` | [test](../tests/test_voxfriction.py) | [conf](../tools/terrain/conformance_voxfriction.txt) | [brief](../docs/voxfriction_brief.md) |
| [`voxbreak.py`](../tools/terrain/voxbreak.py) | `URDRVXZ1` | THE BREAK-EVEN LEDGER — and the gate that was proposed is the WRONG one. THE INEQUALITY HAS NO SOLUTION ON THIS LOOP: five admission rules keyed on at most N distinct owners, every one dearer than the committed reference of 12121714 (`none` 22290004, `one` 19280796, `three` 19008949, `all` 19037173), the best still 57% over. AND THE SINGLE-OWNER GATE LOSES TO NO GATE AT ALL by 243623, because `voxfriction`'s 2- and 3-owner buckets are POSITIVE too and a single-owner gate declines them while still paying the admission read — sharing the majority of a benefit is not being the whole of it. Six accounts NEVER FUSED; both baselines are COMMITTED numbers, so this is a DECOMPOSITION and not a second measurement. The best gate and NO gate EXECUTE THE IDENTICAL 18889124 operations — the gate does not choose which certificates to EARN, it predicts which attempts are DOOMED. THE DEFICIT IS THE SCAFFOLDING: the certificate retires 3281055 while the tiled loop costs 10168290 over the reference before any certificate is consulted. No prediction is scored; the TILE-SIZE pre-registration ships one commit early | `voxbreak` | [test](../tests/test_voxbreak.py) | [conf](../tools/terrain/conformance_voxbreak.txt) | [brief](../docs/voxbreak_brief.md) |
| [`voxschism.py`](../tools/terrain/voxschism.py) | `URDRVXX1` | THE POPULATIONS ARE REAL AND NO FREE SIGNAL SELECTS THEM — a CENSUS of four strategies on every one of 1728 tiles, testing whether the workload partitions into populations with different positive net margins. YES ON THE POPULATIONS: `steno1` wins 349 tiles all with ONE owner, `stenoN` 35 all with two or three, `reference` 1344 — disjoint winning sets — and the TILED TRAVERSAL WINS ZERO, never the best strategy for a single tile anywhere, which is the mechanism behind `voxbreak`'s scaffolding tax. The reference is ATTRIBUTED tile by tile for the first time, a partition summing exactly to the committed 12121714. NO ON THE SELECTION: a hindsight ORACLE is 11% under the reference — the first thing in this arc to get under it — but every free signal captures EXACTLY ZERO of that margin at every resolution from 7 groups to 68, because inside the best population `steno1` wins 349 by 1235531 and loses 222 by 1700567. A FRAME-INDEX PLANT finds +280476, proving the zeros are a measurement and not an inability. No rule is frozen | `voxschism` | [test](../tests/test_voxschism.py) | [conf](../tools/terrain/conformance_voxschism.txt) | [brief](../docs/voxschism_brief.md) |
| [`voxtile.py`](../tools/terrain/voxtile.py) | `URDRVTL1` | THE TILE SIZE WAS NEVER A TUNING PARAMETER — IT WAS THE ANSWER. Every rung from `voxcond` on concluded the arrangement loses to the reference, and every one measured at ONE tile size nobody varied. Swept over eight sizes, all dividing 96 and 72: THREE SPEND LESS THAN THE COMMITTED REFERENCE of 12121714 (tile 1 11245507, TILE 2 10820264, tile 3 11672193) — the best 10.7% under, byte-identical on all sixteen states, ONE strategy and NO selector, the first buildable thing in this arc to beat `voxref`. THE FIRST PASS WAS WRONG AND THAT IS THE POINT: uncharged the UNIT tile looked 19.5% under, cheating in five places the untiled reference never pays (range/index/owners/visit/complete), worth 1492722 at tile 1 against 416938 at tile 8 — 3.5x more to the arm that was winning. TWO ANCHORS: tile 1 costs EXACTLY the reference bare, tile 8 reproduces `voxbreak`'s 22290004 and 19037173 to the operation. Nothing is retracted — the earlier verdicts are RUN here and still bite at their own tile. 4 of 5 pre-registered predictions HIT, including the one named most likely to miss | `voxtile` | [test](../tests/test_voxtile.py) | [conf](../tools/terrain/conformance_voxtile.txt) | [brief](../docs/voxtile_brief.md) |
| [`worldbind.py`](../tools/terrain/worldbind.py) | `URDRWBD1` | An authored world bound to certified ground — the Ursprung seam's two silent corruptions (float coordinates, flipped up-axis) shut as doors, ground checked across languages, content-addressed chunks with content split from provenance | `worldbind` | [test](../tests/test_worldbind.py) | [conf](../tools/terrain/conformance_worldbind.txt) | [brief](../docs/worldbind_brief.md) |
| [`scenecost.py`](../tools/terrain/scenecost.py) | `URDRSCN1` | The composed scene's price as evidence — two sweeps x three configurations + container chains, cross-OS identity for avatar and sky, and the RESOLUTION law (a verdict whose margin sits inside the instrument's own spread is FITS but unresolved) | `scenecost` | [test](../tests/test_scenecost.py) | [conf](../tools/terrain/conformance_scenecost.txt) | [brief](../docs/scenecost_brief.md) |
| [`rescell.py`](../tools/terrain/rescell.py) | `URDRRSC1` | The resolution ladder as evidence — two independent three-cell sweeps, verdicts agreed at 120 Hz (720p certified ceiling, 1080p EXCEEDS), the conservative pair law at 60 Hz, the convexity caution vindicated in bytes | `rescell` | [test](../tests/test_rescell.py) | [conf](../tools/terrain/conformance_rescell.txt) | [brief](../docs/rescell_brief.md) |
...
**The 3D representation arc, and then the instrument the arc needed.** The question was whether a
richer world representation makes rollback cheaper, and answering it honestly cost more rungs than
asking it: `contact` → `stride` → `lift` → `vantage` → `framing` build the representation and
measure what a certified identity loses on the way into it; `vouch` → `retain` → `mould` ask
whether rollback can reproduce the exact *reason* an actor was grounded and what a snapshot must
keep for it to; `measure` states the admission law for a performance claim (workload, host,
denominator, baseline — all NAMED) and `rollbench` is the instrument `measure` could not contain,
with a clock, a log, and a structural refusal to grade its own output.

Then the first real host log refused, and every rung after it exists because a defect surfaced
rather than because it was planned. `reachable` (a gate must admit something its own producer can
make) → `retire` (a retired law names its successor, and nothing outside its module may call it) →
`entry` (a door that cannot refuse turns a flag into a filename) → `confound` (a treatment axis
iterated outermost is a proxy for elapsed time) → `repeat` (variance has levels, and iterations
sample the wrong one) → `deeper` (a timing difference with no counted difference is UNEXPLAINED) →
`attest` (the measurement becomes an artifact the gate re-reads, every figure derived from sealed
bytes) → `pedigree` (integrity is not provenance) → `rehearse` (plausible is not reproducible) →
`indexed` (a gated module appears in the tree's own index) → `reflow` (a line break is not a claim).

The answer, on the named host across five independent executions: **moulding costs a CONSTANT
penalty per rollback**, flat across a depth axis over which the replay work grows six- to
twenty-four-fold — it moves the intercept and cannot move the slope, which is what `measure`'s
a-priori op-count predicted before any clock was started.

## Every file

126 modules. Gate stage `terrain` covers `heightfield` + `terrain_bridge`; `bench` is deliberately
...
| [`attest.py`](../tools/terrain/attest.py) | `URDRATT1` | The measurement becomes an ARTIFACT the gate re-reads — every figure derived from the sealed bytes, never typed | `attest` | [test](../tests/test_attest.py) | [conf](../tools/terrain/conformance_attest.txt) | [brief](../docs/attest_brief.md) |
...
| [`contact.py`](../tools/terrain/contact.py) | `URDRCON1` | Ground contact as a certified state, with the reason it holds (T3D-1) | `contact` | [test](../tests/test_contact.py) | [conf](../tools/terrain/conformance_contact.txt) | [brief](../docs/contact_brief.md) |
...
| [`deeper.py`](../tools/terrain/deeper.py) | `URDRDPR1` | A timing difference with NO COUNTED DIFFERENCE is UNEXPLAINED — Ousterhout's rule, mechanized | `deeper` | [test](../tests/test_deeper.py) | [conf](../tools/terrain/conformance_deeper.txt) | [brief](../docs/deeper_brief.md) |
...
| [`entry.py`](../tools/terrain/entry.py) | `URDRENT1` | An entry point is a door, and a door that cannot refuse turns a flag into a filename | `entry` | [test](../tests/test_entry.py) | [conf](../tools/terrain/conformance_entry.txt) | [brief](../docs/entry_brief.md) |
...
| [`framing.py`](../tools/terrain/framing.py) | `URDRFRM1` | Does this world fit in this frame — the coverage prediction `horizon` could not make | `framing` | [test](../tests/test_framing.py) | [conf](../tools/terrain/conformance_framing.txt) | [brief](../docs/framing_brief.md) |
...
| [`indexed.py`](../tools/terrain/indexed.py) | `URDRIDX1` | A gated module appears in the tree's own index: current COUNTS are not a current DOCUMENT | `indexed` | [test](../tests/test_indexed.py) | [conf](../tools/terrain/conformance_indexed.txt) | [brief](../docs/indexed_brief.md) |
...
| [`lift.py`](../tools/terrain/lift.py) | `URDRLFT1` | How much of a certified identity survives being carried into a richer representation | `lift` | [test](../tests/test_lift.py) | [conf](../tools/terrain/conformance_lift.txt) | [brief](../docs/lift_brief.md) |
...
| [`measure.py`](../tools/terrain/measure.py) | `URDRMSR1` | A performance claim is valid only when its workload, host, denominator and baseline are NAMED | `measure` | [test](../tests/test_measure.py) | [conf](../tools/terrain/conformance_measure.txt) | [brief](../docs/measure_brief.md) |
...
| [`mould.py`](../tools/terrain/mould.py) | `URDRMLD1` | The record takes the shape of the state — the saving is a TYPE rather than a policy | `mould` | [test](../tests/test_mould.py) | [conf](../tools/terrain/conformance_mould.txt) | [brief](../docs/mould_brief.md) |
...
| [`pedigree.py`](../tools/terrain/pedigree.py) | `URDRPDG1` | A claim may only consume an artifact whose PROVENANCE is admissible; the artifact is its own primary witness | `pedigree` | [test](../tests/test_pedigree.py) | [conf](../tools/terrain/conformance_pedigree.txt) | [brief](../docs/pedigree_brief.md) |
...
| [`retain.py`](../tools/terrain/retain.py) | `URDRRTN1` | What a snapshot must keep for a replay to reproduce the same REASONS | `retain` | [test](../tests/test_retain.py) | [conf](../tools/terrain/conformance_retain.txt) | [brief](../docs/retain_brief.md) |
| [`retire.py`](../tools/terrain/retire.py) | `URDRRET1` | A retired law names its successor, and nothing outside its own module may call it | `retire` | [test](../tests/test_retire.py) | [conf](../tools/terrain/conformance_retire.txt) | [brief](../docs/retire_brief.md) |
| [`rollbench.py`](../tools/terrain/rollbench.py) | `URDRRBN1` | The instrument `measure` could not contain — it has a clock, emits a log, and REFUSES to grade its own output | `rollbench` | [test](../tests/test_rollbench.py) | [conf](../tools/terrain/conformance_rollbench.txt) | [brief](../docs/rollbench_brief.md) |
...
| [`stride.py`](../tools/terrain/stride.py) | `URDRSTR1` | The 3D deterministic tick — the first law that consumes `contact` | `stride` | [test](../tests/test_stride.py) | [conf](../tools/terrain/conformance_stride.txt) | [brief](../docs/stride_brief.md) |
...
| [`vantage.py`](../tools/terrain/vantage.py) | `URDRVAN1` | The first-person frame, and the eye is TAKEN rather than derived | `vantage` | [test](../tests/test_vantage.py) | [conf](../tools/terrain/conformance_vantage.txt) | [brief](../docs/vantage_brief.md) |
...
| [`vouch.py`](../tools/terrain/vouch.py) | `URDRVCH1` | Can rollback reproduce the exact REASON the actor was grounded | `vouch` | [test](../tests/test_vouch.py) | [conf](../tools/terrain/conformance_vouch.txt) | [brief](../docs/vouch_brief.md) |
| [`voxlat.py`](../tools/terrain/voxlat.py) | `URDRVXF1` | The certified integer voxel lattice (S1) — clz LCA depth, attained 4*B^3 | `voxlat` | [test](../tests/test_voxlat.py) | [conf](../tools/terrain/conformance_voxlat.txt) | [brief](../docs/voxlat_brief.md) |
...
**MEASURED, as of this writing.** 168 modules under `tools/terrain/`, 251 suites, 4037 unit
falsifiers with 0 red, 1117 gate rows, 0 FAIL. The gate prints `GATE PASSED` twice byte-identically
...
Two of those four figures used to be WRONG here — the row total sat at 896 against a live 964, and
the module count at 123 against a live 125 — while both documentation guards ran green. This file
was not in `doc_currency`'s enforced list, and once added the guard still could not see either
number: the falsifier idiom was hard-wrapped mid-phrase and the pattern carried a literal space, and
the plain-English phrasing of the row total matched no pattern at all. Both holes are closed
(`reflow`, URDRRFL1) and this paragraph is now enforced like the rest — which is also why it no
longer reproduces either idiom verbatim, since a document that quotes a count idiom is read as
claiming it.
