...
# tools/frontfps/fpquat_rs — Rust placement of `tools/frontfps/fpquat.py`

## Index

- `fpquat.rs` — the single translation unit: an independent Rust re-implementation of
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the quaternion battery and the wrap64 defect, std-only, on the owner's Windows/rustc host. The pinned
...
- Build & run: `rustc -O fpquat.rs -o fpquat && ./fpquat`
