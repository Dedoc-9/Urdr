...
# tools/frontfps/fpquat_c — C99 placement of `tools/frontfps/fpquat.py`

## Index

- `fpquat.c` — the single translation unit: an independent C99 re-implementation of
  `tools/frontfps/fpquat.py` (URDRFPQ1 Q32.32 rotation substrate). Self-contained (own SHA-256, own fixed-point/integer arithmetic);
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the quaternion battery row-for-row (products in i128, never wrapped, i64 REFUSE ceiling) and the wrap64 defect. The pinned
...
- Build & run: `cc -O2 -std=c99 -Wall -Wextra fpquat.c -o fpquat && ./fpquat`
...
- The reference and the red-first gate stage live in the parent module (`tools/frontfps/fpquat.py`); this
