...
divisions/pose-sample, fppose 77/world-transform, fpview 332 bytes/scene. The
bench protocol (`docs/bench_protocol.md`) is law: named host (ROG Ally X), display
segregation, sealed metric, `cold ≠ sustained`. What's missing is a composed
per-tick work model and the wall-clock run — and only the named host can produce
the second.

**Orient.** Stage 7 splits along the honesty line the roadmap drew: the repo can
measure WORK exactly (host-independent operation counts) but never TIME (that
needs silicon). So the gate-able artifact is `frontbench` — the exact
frozen-division count for the canonical 100-biped sim tick, composed from the
measured proxies and cross-checked against instrumented execution. Milliseconds
and fps stay NOT_MEASURED, and — new here — that boundary is itself a gate row: no
budget entry may read MEASURED without a §3 host log.

**Decide.** First increment: `frontbench` (URDR-FPSW-BENCH-1) —
`sim_tick_divisions(100) = 100 × (55 + 77) = 13200` frozen divisions, pinned; the
composition cross-check (model == real instrumented execution == the module
proxies); the drop-animation defect; and the budget-honesty gate (no perf entry
MEASURED without a host log; a dishonest manifest caught). Deferred by law: the
wall-clock `--measure` (owner-only, on the Ally X), the SOA native placement,
streaming-editor integration, and every ms/fps number.
