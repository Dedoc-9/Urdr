...
# tools/frontfps/fpclip_c — C99 placement of `tools/frontfps/fpclip.py`

## Index

- `fpclip.c` — the single translation unit: an independent C99 re-implementation of
  `tools/frontfps/fpclip.py` (URDRCLP1 pose/clip sampler). Self-contained (own SHA-256, own fixed-point/integer arithmetic);
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the idle/walk/twist clip-sample goldens and the sprint-tick defect that must diverge. The pinned
...
- Build & run: `cc -O2 -std=c99 -Wall -Wextra fpclip.c -o fpclip && ./fpclip`
...
- The reference and the red-first gate stage live in the parent module (`tools/frontfps/fpclip.py`); this
