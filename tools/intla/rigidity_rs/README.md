...
# tools/intla/rigidity_rs — Rust placement of `tools/intla/rigidity (via urdr-math)`

## Index

- `rigidity.rs` — the single translation unit: an independent Rust re-implementation of
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the four certificates and the overflow REFUSE, bit-for-bit, on Windows/rustc. The pinned
...
- Build & run: `rustc -O rigidity.rs -o rigidity && ./rigidity`
