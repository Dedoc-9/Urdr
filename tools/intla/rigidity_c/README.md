...
# tools/intla/rigidity_c — C99 placement of `tools/intla/rigidity (via urdr-math)`

## Index

- `rigidity.c` — the single translation unit: an independent C99 re-implementation of
  `tools/intla/rigidity (via urdr-math)` (exact 2D rigidity certificate). Self-contained (own SHA-256, own fixed-point/integer arithmetic);
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces four rigidity certificates (RIGID/FLEXIBLE + rank + dof) and the overflow REFUSE, bit-for-bit. The pinned
...
- Build & run: `cc -O2 -std=c99 -Wall -Wextra rigidity.c -o rigidity && ./rigidity`
...
- The reference and the red-first gate stage live in the parent module (`tools/intla/rigidity (via urdr-math)`); this
