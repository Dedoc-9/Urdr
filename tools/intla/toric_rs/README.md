...
# tools/intla/toric_rs — Rust placement of `tools/intla toric detector`

## Index

- `toric.rs` — the single translation unit: an independent Rust re-implementation of
  `tools/intla toric detector` (toric-code homology detector (D17)). Self-contained (own SHA-256, own fixed-point/integer arithmetic);
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the toric digest and k = dim H1 = 2*genus against the Python reference and the C99 placement, bit-for-bit. The pinned
...
- Build & run: `rustc -O toric.rs -o toric && ./toric`
...
- The reference and the red-first gate stage live in the parent module (`tools/intla toric detector`); this
