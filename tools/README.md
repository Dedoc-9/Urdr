...
(`../verify.py`): **879 unit falsifiers / 539 rows**, run twice, bit-identical.
...
- Run the whole gate from the repo root: `PYTHONHASHSEED=0 PYTHONUTF8=1 python verify.py`
  (expect `GATE PASSED` — 879 unit falsifiers / 539 rows). Each module's README documents running it standalone.
