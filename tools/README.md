...
(`../verify.py`): **822 unit falsifiers / 511 rows**, run twice, bit-identical.
...
- Run the whole gate from the repo root: `PYTHONHASHSEED=0 PYTHONUTF8=1 python verify.py`
  (expect `GATE PASSED` — 822 unit falsifiers / 511 rows). Each module's README documents running it standalone.
