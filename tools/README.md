...
(`../verify.py`): **855 unit falsifiers / 527 rows**, run twice, bit-identical.
...
- Run the whole gate from the repo root: `PYTHONHASHSEED=0 PYTHONUTF8=1 python verify.py`
  (expect `GATE PASSED` — 855 unit falsifiers / 527 rows). Each module's README documents running it standalone.
