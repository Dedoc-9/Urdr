...
- [`terrain/`](terrain/) — the certified **terrain & wave studio**: an exact `URDRHF1`
  ...
  **observer** (a view is admitted iff it reconstructs to the current pose, else refused — replay and
  forgery caught), a declared-but-cited WebGL2 view behind `view_witness`, and the `heightfield_rs`
  cross-placement re-verified live. The observer + foundation of FPS movement over the certified field.
...
(`../verify.py`): **656 unit falsifiers / 440 rows**, run twice, bit-identical.
...
- Run the whole gate from the repo root: `PYTHONHASHSEED=0 PYTHONUTF8=1 python verify.py`
  (expect `GATE PASSED` — 656 unit falsifiers / 440 rows). Each module's README documents running it standalone.
