...
(`../verify.py`): **915 unit falsifiers / 563 rows**, run twice, bit-identical.
...
- Run the whole gate from the repo root: `PYTHONHASHSEED=0 PYTHONUTF8=1 python verify.py`
  (expect `GATE PASSED` — 915 unit falsifiers / 563 rows). Each module's README documents running it standalone.
