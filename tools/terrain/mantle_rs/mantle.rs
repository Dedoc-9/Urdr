...
//
// mantle_rs — THE TILE PATH, PLACED: vista's frame (one exact ray per column through the URDRVXR1
// traversal, the strip and the floor as floors of rationals, an 8-bit URDRFB1 index frame) and
// mantle's picture (exact texture coordinates on a flat tile, the table's own per-band maps applied
// to the texel), ported std-only, as ONE binary that reads a scene and prints two witnesses:
//
...
//
// STUDIO-0. This placement exists to answer one question with a number: how far a native kernel of
// the certified renderer is from a 1080p frame budget, on a named host, with the pixels bit-identical
// to the Python oracle. It ports NOTHING of the world: the level's cells, the eye, the facing, the
// colour table, the per-band maps and the tiles arrive as INPUT (written by gen_vectors.py from the
// Python modules), so the kernel is exactly the per-frame work — 1920 traversals, the strip and floor
// fills, 2,073,600 texel lookups. Its correctness is not argued: the gate row `mantle-placement`
// compiles this file live and compares both witnesses to the Python modules' own values over the
// corpus scenes, with a mutated sign table shown to diverge.
//
...
//
//     rustc -O mantle.rs -o mantle
//     ./mantle scene.bin                 # the two witnesses (+ selfcheck OK)
//     ./mantle scene.bin --bench 200     # off-gate: p50/p95/p99/max microseconds per phase, host named
//
// Input (little-endian): "URDRMNTI" | u32 w | u32 rows | w*rows cells | i32 pos_x | i32 pos_z |
// u8 facing (0 N, 1 E, 2 S, 3 W) | u32 depth | 768 table | 32*256 wall_map | 32*256 floor_map |
...
fn read_scene(path: &str) -> Scene {
    ...
    if take(&mut p, 8) != MAGIC_IN {
        refuse("input magic is not URDRMNTI");
    }
    ...
    let facing = take(&mut p, 1)[0];
    if facing > 3 {
        refuse("facing must be 0..3");
    }
    ...
    Scene { w, rows, cells, pos_x, pos_z, facing, table, wall_map, floor_map, walls, floor }
}

// ------------------------------------------------------------------ vista: the camera and one column's strip
fn direction(facing: u8, column: usize) -> (i64, i64) {
...
impl Scene {
    ...
    fn first_hit(&self, ex: i64, ez: i64, dx: i64, dz: i64) -> Option<(i64, i64, u8, i64, i64)> {
        let n = LATTICE;
...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!("usage: mantle <scene.bin> [--bench N] [--warm M]");
        exit(2);
    }
