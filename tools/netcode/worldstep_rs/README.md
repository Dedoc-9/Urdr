...
# tools/netcode/worldstep_rs — Rust placement of `tools/netcode/worldstep.py`

## Index

- `worldstep.rs` — the single translation unit: an independent Rust re-implementation of
  `tools/netcode/worldstep.py` (urdr-netcode N4 authored worlds). Self-contained (own SHA-256, own fixed-point/integer arithmetic);
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the arena-equivalence chain (N4 with no statics equals the frozen N1 arena) and the highway golden. The pinned
...
- Build & run: `rustc -O worldstep.rs -o worldstep && ./worldstep`
...
- The reference and the red-first gate stage live in the parent module (`tools/netcode/worldstep.py`); this
