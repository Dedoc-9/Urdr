...
# tools/netcode/rollback_rs — Rust placement of `tools/netcode/rollback.py`

## Index

- `rollback.rs` — the single translation unit: an independent Rust re-implementation of
  `tools/netcode/rollback.py` (urdr-netcode N2 rollback). Self-contained (own SHA-256, own fixed-point/integer arithmetic);
...
real only if an *independent* implementation reproduces it bit-for-bit. It reproduces the late-delivery rewind/replay golden fea3b967 at K=4 and K=8. The pinned
...
- Build & run: `rustc -O rollback.rs -o rollback && ./rollback`
...
- The reference and the red-first gate stage live in the parent module (`tools/netcode/rollback.py`); this
