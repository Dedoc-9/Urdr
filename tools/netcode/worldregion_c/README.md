...
# tools/netcode/worldregion_c — C99 placement of `tools/netcode/worldregion.py`

## Index

- `worldregion.c` — the single translation unit: an independent C99 re-implementation of
...
real only if an *independent* implementation reproduces it bit-for-bit. An independent build with its own Q32.32 backend, reproducing the Seam Composition golden (regional reunification equals the monolith bit-for-bit). The pinned
...
- Build & run: `cc -O2 -std=c99 -Wall -Wextra worldregion.c -o worldregion && ./worldregion`
