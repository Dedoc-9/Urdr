...
| **N6** the session | [`session.py`](session.py) | `URDRSES1` — **membership is a property of the EVENT, not of the room.** `Session` **is** `WorldPeer` with one precondition: a CALENDAR of `(peer, join, leave)` intervals plugged into `_admit`, the seam the N2 time law already entered through. An event from a peer who *was* a member when it happened and is *not* one now must be admitted on the **event's** tick — evaluating membership at the receiver's HEAD makes admission depend on delivery order, so the identical eleven envelopes give **3 distinct chains instead of 1**, and it fails silently because each peer stays internally consistent. Departure by roster EVICTION lands on the *same* wrong world, digest for digest; only the typed refusal separates them (`AUTH-REFUSE` against `SESSION-REFUSE`). Segmentation survives a changing peer set (0 of 53 cuts); what the snapshot must carry is DERIVED from the AST across the inheritance seam. [brief](../../docs/session_brief.md) | `session` |
