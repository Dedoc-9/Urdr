...
We report a concrete reproducibility result: 24 independent, single-file Rust
...
The 24 Rust and 13 C99 placements share no code, language, or SHA-256 implementation with the
...
The gate runs, deterministically: **754** unit falsifiers; **42** example programs
...
24 independent Rust placements (and 13 C99 runtimes) reproduce the reference's state, frame, physics,
...
bit-for-bit on fixed corpora, behind a 754-test gate with typed
refusa