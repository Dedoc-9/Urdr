...
We report a concrete reproducibility result: 23 independent, single-file Rust
...
in the netcode stack the placements agree on the *defect* digests as well. A 578-test
...
The 23 Rust and 13 C99 placements share no code, language, or SHA-256 implementation with the
...
The gate runs, deterministically: **578** unit falsifiers; **42** example programs
...
23 independent Rust placements (and 13 C99 runtimes) reproduce the reference's state, frame, physics,
...
bit-for-bit on fixed corpora, behind a 578-test gate with typed
refusa