...
We report a concrete reproducibility result: 25 independent, single-file Rust
...
The 25 Rust and 14 C99 placements share no code, language, or SHA-256 implementation with the
...
The gate runs, deterministically: **813** unit falsifiers; **42** example programs
...
25 independent Rust placements (and 14 C99 runtimes) reproduce the reference's state, frame, physics,
...
bit-for-bit on fixed corpora, behind a 813-test gate with typed
refusa