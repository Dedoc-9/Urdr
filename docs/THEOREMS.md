...
| T20 | **Criticality is deterministic, conservative, and self-limiting (urdr-criticality).** A branching-diffusion field (Galton-board flux-form transport, keff multiplication, Doppler feedback): transport conserves population EXACTLY; keff=1 stationary / keff<1 decays / keff=2 unregulated `FIELD-REFUSE`s at the bound; and Doppler feedback regulates a supercritical `k0=2.0` to a bounded steady state — reactor stability, reproducibly. **And the steady state is now the CLOSED FORM rather than a digest**: `n* = (k0−1)·n_ref` per cell, read across five (k0, n_ref) pairs — linear in each argument, exact at fractional `k0` — because a trace digest is an IDENTITY and a law has a SHAPE. Two findings follow from writing it down: **Doppler MOVES the critical point** (`k0=1` is stationary bare and decays through the regulator, whose closed form rests it at zero), and **the excursion is DOWNWARD** (one turning point, and the run's maximum is the rest point — there is no peak). | `criticality`: Galton golden `064f7cfc…` + Doppler steady-state golden `8439d5a6…` + closed-form panel `a66553e0…`; exact-conservation over 50 gens; every panel member on the rational rest point to the LAST BIT of Q32.32 (3 exact, 2 one ulp low, none high — `mul_k` truncates — and the deficit is a fixed point, identical at 100 and at 800 generations); two non-vacuity defects (drop Doppler → explode → `FIELD-REFUSE`; scale the feedback density by two → bounded, steady, never refuses, and rests on half the right number). MEASURED reference; bounded regime; cross-placement DECLARED. Honest scope: 1D, one-group, rational Doppler (physical √T Doppler is irrational → refuse regime); the panel reads linearity in both arguments and one fractional `k0`, so a law departing from the closed form outside their span is invisible here. |
...
The gate enforcing all of the above: **4571 unit falsifiers + per-layer conformance
...
on Windows and Linux. 34 std-only Rust placements, 14 C99 runtimes.
