...
verification regime). The first eleven have LANDED: `move` (rung 1) produced the first authoritative
...
`persist.py`), and `enact` (rung 10) LANDED as the typed action authority — a KIND, a payload and one dispatch binding each recorded action to the transition that owns it, the prerequisite `replay` was missing — and `replay` (rung 11) LANDED as the from-origin certification that the recovered history reproduces the independently stored savegame, closing Slice B by reconstructing the origin rather than by a new record — one rung remains before KINEMA. Working names only for the unbuilt rungs; the tree's rule is
...
| 12 | **statecanon** | §2 the assembled snapshot | all of the above (incl. `replay`) | seed + world identity + dungeon + entity + RNG + action history digested as one `D_n`; a view quantity in the digest reddens; the snapshot round-trips |
