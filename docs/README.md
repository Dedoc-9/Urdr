...
| `PAPER.md` | **OSDI-style systems paper** for the deterministic execution pipeline — problem, layered design, implementation, and the cross-implementation reproducibility evaluation (thirteen independent Rust placements + four C99 runtimes — the math spine, the netcode stack, and the toric/rigidity detectors — reproducing the kernel, frame, physics, field/Marangoni/loop, exact-math, fixed-point-dynamics, the full netcode-stack + regional-composition, and two invariant-detector digests behind a 412-test gate; current totals live in `spec/D5-ledger.md`). Scoped to corpus agreement, not universal correctness. |
