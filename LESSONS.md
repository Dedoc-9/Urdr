...
| L7 | Capture nondeterminism at the boundary, never fake it away (clocks, RNG, floats, external reads are *inputs*). | v0.1 simply has no such boundary: the core is pure, integer-only, effect-free. When effects arrive they arrive as typed capabilities, recorded as inputs. | grammar (no effect syntax exists yet); ledger marks effects `SCOPED` |
