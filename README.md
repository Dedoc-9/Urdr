...
across independent implementations or explicitly refused. **34** single-file Rust placements
...
on fixed corpora, behind a **3277-test gate** — and the math spine, the netcode region, the frontfps ladder, and the toric/rigidity/homology detectors carry **14** C99 placements, so
...
## What exists today — `IMPLEMENTED / MEASURED` via the gate (rungs R0–R6b · M5–M7 · P1–P6 · N1–N5 · N4.1 · D14–D17 · D19's W1 · the frontfps FPS/MMO ladder (Stages 1–7) · the `URDRPD1` topological homology / OOB witness + the invariant-detector library · **Hainuwele**, the manifold MMO arc (T1–T3.56, Stages A–I + the sealed wire, visible-world and mesh phases — named for the Wemale dema-deity from whose buried, divided body the living crops grew: here, every living capability grows from content-addressed records buried under their own digests): movement → latency → storage → recovery → streaming → the regional cut → the mutable world → certified concurrency → authority nullity → the standing lease → durable intent → the representation theorem → equal-or-refuse replication → the adversarial-transport loom → the signed wire → interest shift → the reality attestation → the windowed loop → the wired window → the actor wire → the sealed frame → the attested session → the certified mesh (MESH == MONOLITH) → the visible-evidence arc: a playable conformance demo (`fpsdemo` v1.12 — certified camera + canon terrain, cross-OS replay chains, the frozen 720p/120 Hz competitive point, a composed far-field sky) with its measurements sealed as gate-read records (reach envelope, cap cost, sky price, resolution ladder) and the `hainuwele/v2` distance cores (regions → LOD → planet horizon → far field) under their own fast gate)
...
sealed bench on a named host — this repo has never made a timing claim, and does not start now. The second
step, `glide` (MMO Stage B opener), lifts that live loop off the grid: the same input log folds into Q32.32
**sub-cell** poses (one micro-step is `ONE>>k`, an exact shift, so `sub` of them a cell), and the keystone is
...
even from a sub-cell wall-stopped pose, the hard case). On that foundation, `cpredict` closes the Stage-A ×
Stage-B loop: client prediction reconciled on the *continuous* trajectory — `reconstruct == glide(auth)`
bit-for-bit — and provably **sharper** than the grid it refines. Because floored glide == drive, the
continuous localizer catches a misprediction no later than the discrete `predict`, and *strictly* earlier on
a sub-cell mispredict the grid cannot see: a walk guessed where the authority sprinted into a wall lands in
the same cell but a different sub-cell pose, so `cpredict` rolls back where `drive` saw nothing — while still
flooring to `drive(auth)`, refining the discrete reconcile without ever contradicting it. On that loop,
`interest` opens **Stage C** — the scale layer: which peers even need to hear about which actors. An exact
...
theorem about the terrain's topology rather than a watchdog on the wire. `crosswarden` then closes the Stage D+E synthesis: the handoff seam itself is made cheat-proof — a boundary crossing is certified against the *merged* authority (`F_A` west of the split, `F_B` east, the same world the handoff produces), so a shard-local warden checking only its own stale view is provably insufficient; the through-wall exploit that slips past shard A is refused by the merge. And `dirward` refines the topological check from undirected components to directed reachability, so a one-way cliff is honest: the undirected warden treats a cliff as a solid wall — false-refusing the legal descent and unable to tell a one-way drop from a wall — while directed reachability admits the descent and refuses only the climb-back (`WARD-ONEWAY`, distinct from a wall's `WARD-UNREACH`). And `wardhom` closes the loop the three wardens left open: their β₀ — the component count the topological check rests on, computed directly by union-find — is proved equal to URDRPD1's 𝔽₂-homology rank H₀ and reproduced bit-for-bit in Python, C99, and Rust, so the anti-cheat's topological certificate is a genuinely cross-placed invariant rather than a single pass — and it now covers all three wardens: crosswarden's merged-region β₀ is the same cross-placed homology invariant (a pinned `merge8` scene), while dirward's directed `num_scc` is bounded by it (`num_scc <= β₀`, strict where a directed cycle fuses undirected components — a gap F₂ homology cannot see, pinned by a minimal witness). Then `opcost` + `bench` open **Stage H** — the latency envelope: each operation's exact integer WORK is gate-certified (deterministic, MEASURED) while `bench.py` measures the wall-clock on a NAMED host (MEASURED-on-named-host), so the pervasive `NOT_MEASURED` becomes a two-layer measurement — a certified op-count times a host-tagged per-op cost — with a work-budget `OPCOST-REFUSE` seeding the guarantee. `govern` then makes that budget BIND every tick — admitting a FIFO prefix of actors within the op-budget and deferring the rest — with three certified guarantees: never-overrun (a tick's work is always <= budget, so its wall-clock is bounded by the benched per-op cost), progress (a single over-budget actor is refused, so every tick admits at least one and drains in <= N ticks), and bounded-wait (in-order, so no actor starves). `priogov` then adds priority with aging (highest first, but a bounded wait so the lowest never starves), `horizon` bounds the rollback-reconcile window (a client correction admits within H boundaries — byte-exact — or `HORIZON-REFUSE`, so the worst-case reconcile latency is exactly H), and `slo` states the whole guarantee as one certified number (worst-case latency = admission wait + rollback window, with a `SLO-REFUSE` when a config can't meet its target and a checked soundness that the bound never under-counts the real governor). `clslo` then makes that single number **per-class**: each priority tier gets its own certified worst-case (`ceil(N>=p / floor(budget/cost)) + H`), so a premium tier waits behind fewer actors than free, the one-class case collapses back to `slo`'s uniform bound, and each tier's number is checked EQUAL to `priogov`'s real per-class drain — a by-name `CLSLO-REFUSE` when a tier cannot meet its target, so `priogov`'s weighted fairness stops being a bare mechanism and becomes an accountable guarantee. Finally `storecost` bounds the one thing the arc never had — SPACE: an H-deep rollback window costs (H+1) boundary snapshots at 4 + 25*N bytes each, a byte count checked EQUAL to a real serialization (round-trip byte-exact), with a `STORAGE-REFUSE` when that window would exceed a memory budget — so the latency guarantee stops silently depending on unbounded storage (a time bound with no space bound is half a promise). And `persist` makes that priced window DURABLE — the same H+1 boundary snapshots as content-addressed checkpoint records (record = magic | boundary | canonical payload | SHA-256, the ONE digest serving as integrity check, content address, and on-disk filename — the membrane's identity law) plus a manifest that binds the boundaries in order, restored bit-for-bit through a real directory round-trip or refused: every single-byte flip, truncation, rename, and substitution is a typed `PERSIST-REFUSE` (checked exhaustively over a pinned record, with the membership law enforced in two independent layers), and the durable cost is a closed form — exactly `storecost`'s in-memory bound plus a fixed integrity premium — gated by the same `STORAGE-REFUSE` budget law, so the rollback window the whole arc priced now survives the process that wrote it. Then `chunkload` opens **Stage I** — the streaming world: the certified field is cut into content-addressed chunks under the same one-digest-three-roles law (chunk_bytes(C) = 56 + 8·C², a field manifest binding the digest grid), reassembly is byte-for-byte and reproduces the pinned URDRHF1 canon digest — partition, stream, reunify, same witness, the D16 pattern applied to terrain — and movement over a PARTIAL world is equal-or-refuse: a glide over exactly its certified demand set (the chunks the mover actually reads — proved sufficient AND necessary, not a heuristic radius) reproduces the frozen mover bit-for-bit, while any read into an unloaded chunk is a typed `CHUNK-REFUSE` — never unloaded-terrain-treated-as-a-wall, because the loader may not fake a stop it cannot see. The field's own storage — the piece `storecost` and `persist` each deferred — is now a closed form under the same `STORAGE-REFUSE` budget law. And `resurrect` proves the durable window is a RECOVERY substrate, not just a storage format: a process that dies after saving is revived from the store ALONE — the gate spawns a real successor process whose only channel to its predecessor is the disk — and the revived continuation equals the never-died timeline bit-for-bit (the through-disk splice equivalence), a late correction resumes from the saved boundary and converges to the authority exactly (including from a fractional wall-stopped pose; k=0 restarts from the retained start, `cpredict`'s own law made durable), a correction older than the window is the durable `HORIZON`-analog `RESURRECT-REFUSE`, and an integral-but-inconsistent snapshot (a ground that contradicts the floor-sampled authority) is refused, not resumed — the restore-and-replay law production stream processors run on, landed byte-exactly. And `chunkstate` distributes that recovery law: the world's actor snapshot is cut PER REGION (each actor claimed by the region its pose floors into, on `chunkload`'s own grid, identity riding as the global index), each region one content-addressed record bound by a cut manifest — and the reunification reproduces the MONOLITHIC `persist` window byte-for-byte, same records, same manifest, same content addresses (the D16 partition-reunify-same-witness law on dynamic state; the consistent cut is free because the lockstep tick already IS the barrier — Chandy-Lamport with no alignment problem). A seam-crossing actor migrates by per-boundary re-partition, a region recovers independently (its resume equals the global law), and the cut carries the distributed snapshot's anti-cheat shape: a doubly-claimed, lost, annexed, or mixed-boundary actor set is a typed `CHUNKSTATE-REFUSE` — no actor doubly owned, dropped, or annexed. And `terraform` makes the streamed world MUTABLE — the membrane's ☿-law applied to terrain: an edit mints a NEW chunk digest and a NEW field manifest in which EXACTLY one slot moves (every untouched chunk keeps its content address, gate-checked), while the parent world still reassembles bit-for-bit under its old manifest — anamnesis is an address, not an undo. The edit itself is a 96-byte content-addressed record (magic | parent manifest digest | cell | old height | new height | SHA-256) applied compare-and-swap: a record authored against another world is a typed `TERRAFORM-REFUSE` (a stale parent is refused, never rebased), as is an old-height mismatch, an off-grid target, any single-byte corruption, and an out-of-order replay — while replaying the intact edit log reproduces the head manifest bit-for-bit, order structural via the parent chain. The blast radius of an edit is CERTIFIED by `chunkload`'s own demand sets (a walk that reads the edited chunk diverges; a walk that doesn't is bit-identical), an edit's incremental cost is a closed form under the same `STORAGE-REFUSE` budget law, and a durable snapshot whose ground an edit has contradicted refuses on revive via `resurrect`'s own consistency law — the "until live world edits" boundary every storage rung declared is now closed. And `commute` makes that closure a CALCULUS rather than a policy — the proof-object turn: two sibling edits either carry a first-class, content-addressed, independently re-verifiable certificate (233 bytes: the two edit records, a conflict rank, one digest) PROVING order cannot matter, or they refuse. The diamond is discharged constructively — apply A then explicitly-rebased B, apply B then explicitly-rebased A, require field AND manifest equality with the direct two-cell mutation — so nothing is assumed from geometry that isn't re-checked in bytes; rank 0 (different chunks) additionally certifies the blast radii compose without interference, making parallel execution a theorem (every serial order provably equivalent) rather than a permission; rank 1 (same chunk, distinct cells) commutes the world exactly while honestly serializing that chunk's consumers; and the SAME cell twice is `COMMUTE-REFUSE` in two independent layers — certify's cell law and the old-height CAS on the rebased loser — including the no-op-masked construction a repairing closure would wrongly admit. `predict` decides the rank from pure chunk geometry BEFORE any edit exists (concurrency scheduled, not repaired), `closure` replays every permutation of a batch to one head or refuses whole, and `check_certificate` re-derives the entire proof from the parent world, so a forged rank or a tampered record under a fresh seal refuses — certificates are evidence, never authority, and `terraform`'s refusal law stands unweakened beneath the whole calculus. And `rannull` composes the two proof domains into RAN-0 — the authority-nullity certificate, a proof of ABSENCE: `chunkstate` certifies ownership, `commute` certifies semantic independence, and neither alone certifies distributed execution — because the shared authority hid in the binding itself (the global CAS names the world's manifest, which touches every chunk; the reason even rank-0 pairs needed rebases). The 104-byte REGIONAL edit record rebinds the CAS to its chunk's content address — an edit now holds exactly the authority its record names — and RAN-0 certifies Execute(A||B) == Execute(A;B) == Execute(B;A) at three levels of informational absence: the shard is a pure function of one chunk record and one edit record (the frame property — the same chunk embedded in two different worlds produces byte-identical outputs, because influence from outside the authority has no channel to arrive by), the coordinator mints the head manifest from addresses alone (the parent manifest plus two new chunk digests — every other chunk's bytes absent from the store), and the prover discharges the four-way head equality against the global serial law and the commute diamond, with the certificate embedding the ORIGINAL records unchanged — zero rebases, because no shared authority moved. Overlap is `RAN-REFUSE` in two layers proven individually redundant and jointly load-bearing, the commute rank-1 fallback certifies what nullity refuses (strictly stronger, contradicting nothing), recovery inherits the nullity, and the certificate TRANSPORTS across any world that preserves its authorities — synchronization not omitted but proven unnecessary, and distributed write admission becoming a theorem composed from landed laws rather than a new subsystem. And `lease` extends that transport through TIME — proof as an interval, not a moment: an 80-byte standing lease names one chunk state and stays valid until that authority moves; validity is decidable from the manifest alone (one slot — no store, no field), the leased edit admits at EVERY insertion position of a disjoint-authority interval with its bytes unchanged (interval commutation — RAN-0's diamond iterated without re-proving), the cheap admission equals the full global reproof bit-for-bit (the proof paid once at mint, every admission inheriting it), a lease dies at its own use and renews from the new chunk so the lease chain IS the region's write history, and expiry is a typed refusal under the lost-update law — `admit` fetches by the current manifest slot rather than the lease's digest, because the anamnesis store still holds the stale bytes and a stale fetch would silently revert the interval's edits; with both guard layers proven, the lost update is impossible rather than avoided. Optimistic distributed admission with proofs instead of locks: an authority hands out leases, clients author offline, edits admit against any head the interval reaches, and conflict is expiry, typed and honest. And `testament` carries that intent THROUGH DEATH — the write analog of `resurrect`'s replay law: a client may die holding an unadmitted edit, and the 144-byte testament (the regional record under the persist one-digest law — a last will and testimony in one) survives as a content-addressed object a successor can present knowing nothing but the store; probate inherits the entire lease law, exactly-once falls out for free because the admission expires the very lease the testament names, and the refusals speak — "already executed" when the current chunk state IS what the intent produces (adjudicated from the retained parent state, the anamnesis store doing honest read-only work), "distributed" when a foreign edit landed, "unadjudicable" when the evidence is gone — so a successor always learns whether to rest, re-author, or admit ignorance. The death boundary is real (the successor is a subprocess whose argv is a directory and two addresses; its output equals the never-died head twice, bit-identically), every estate object must hash to its filename (an intact object under the wrong address is a substitution, refused), and the executor is pure — a refused probate writes nothing. The remaining staged route (live authoring → a modern depicting client → the full sealed bench) is written down as goals, each to
...
conformance: v12, 36/36 vectors reproduced twice, defect caught 16/16).
...
| [`ANCESTRY.md`](ANCESTRY.md) | The verified four-repository lineage — `executable-epistemics → Dentatus → Ursprung → Urðr` — checked against source, not asserted | — |
| [`AGENTS.md`](AGENTS.md) | The operating contract: how to add a rung, how to run the gate, and the anti-inflation rules that bind an author | — |
...
pipeline is bit-reproducible across independent implementations** (34 Rust placements + 14 C99 runtimes agree with the Python reference on stated corpora), and **a claim cannot outrun its evidence
...
- **Phase V — the MMO stage arc (what the direction became).** The work since Phase IV ran the terrain
  ...
  the placement batch: every rung of the streak independently placed and live-re-verified — then the
  **mutable chunked world** (`terraform`: edits as content-addressed CAS records, exactly-one-slot
  manifests, certified blast radius — the shared "until live world edits" boundary of every storage rung,
  now closed), the **commutation certificate** (`commute`: the proof-object turn — order-independence
  as a first-class, re-verifiable proof derived from terraform's locality, chunkload's demand sets, and
  the CAS; concurrency scheduled by pure geometry, certified in bytes, refused where contested), and
  **RAN-0** (`rannull`: authority nullity — ownership and independence composed into a proof of
  ABSENCE; the regional CAS record, the frame property, the address-only coordinator, the four-way
  head equality, the transportable certificate — distributed write admission as a composed theorem),
  the **standing lease** (`lease`: RAN-0's temporal extension — proof as an interval; state-free
  validity, interval commutation, amortized == reproved, self-expiry + renewal, the lost-update law —
  optimistic distributed admission with proofs instead of locks), and the **testament** (`testament`:
  durable intent — the write that survives its writer; probate through a real successor process,
  ...
  one window law on both layers of the repo). And quintessence — the ID-0 representation theorem —
  CLOSES the calculus: every lawful authority in the five families is characterized by its
  five-axis evidence tuple (historical, spatial, semantic, temporal, identity); the scope finding
  (history and validity are the same address at a scope — world vs chunk — and the difference
  ...
  sound and necessary-with-detection). W2 `storm` is LANDED (the DST loom: convergence-under-chaos, typed chaos, the prefix
  property under loss — the network misbehaves, the gate does not). W3 `sealwrit` is LANDED
  (the signed wire: the record verbatim inside a Lamport-sealed writ, eligibility preceding
  ...
  a named-host `--bench` log, bench_protocol's rule applied to the frame) — and `sealsession`, the
  ATTESTED session (V5, THE VISIBLE-WORLD CAPSTONE: a play session composing loop+wire+ghosts,
  recorded as a self-digested trace the gate replays through the unmodified laws; a forged
  playthrough refuses — the demo becomes a proof). THE VISIBLE-WORLD PHASE (V) IS SEALED: seen,
...
- **Live re-verification of every placement (generalize `heightfield-placement`) — nine stages of
  progress recorded.** The pattern has since generalized across the arc: `latstore-`, `glide-`,
  `streamstate-`, `latarith-`, `writecalc-`, `rollstore-`, `wirephase-`, and `wardhom-placement` now join
...
- **Third-language placements of the remaining layers.** The math spine, the whole netcode stack, and the
  toric and rigidity detectors are multi-runtime (Python + Rust + C99, two OSes — **34 Rust + 14 C99
...
5. **Cross-placement.** A numeric / observer result earns *both placements* only once an independent
   ...
   the gate can compile the port (the nine `*-placement` stages: heightfield, latstore, glide,
   streamstate, latarith, writecalc, rollstore, wirephase, wardhom), that cross-placement is
   **re-verified live every run**, not merely attested once (D17 Axis A).
...
and the matching `*-placement` stage (heightfield, latstore, glide, streamstate, latarith, writecalc,
rollstore, wirephase, or wardhom) reddens
