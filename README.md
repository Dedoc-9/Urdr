...
on fixed corpora, behind a **925-test gate** — and the math spine, the netcode region, the frontfps ladder, and the toric/rigidity/homology detectors carry **14** C99 placements, so
...
- **Phase V — the MMO stage arc (what the direction became).** The work since Phase IV ran the terrain
  ...
  **mutable chunked world** (`terraform`: edits as content-addressed CAS records, exactly-one-slot
  ...
  now closed), and the **commutation certificate** (`commute`: the proof-object turn — order-independence
  as a first-class, re-verifiable proof derived from terraform's locality, chunkload's demand sets, and
  the CAS; concurrency scheduled by pure geometry, certified in bytes, refused where contested). The
  declared arc ahead: the independence lattice, region algebra, and causal witness-sets as queryable
  structures; the queued next rung is the N2-window/durable-window unification.
