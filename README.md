...
across independent implementations or explicitly refused. **33** single-file Rust placements
...
on fixed corpora, behind a **2183-test gate** — and the math spine, the netcode region, the frontfps ladder, and the toric/rigidity/homology detectors carry **14** C99 placements, so
...
pipeline is bit-reproducible across independent implementations** (33 Rust placements + 14 C99 runtimes agree with the Python reference on stated corpora), and **a claim cannot outrun its evidence
...
- **Third-language placements of the remaining layers.** The math spine, the whole netcode stack, and two
  detectors (toric, rigidity) are multi-runtime (Python + Rust + C99, two OSes — **33 Rust + 14 C99
