...
on fixed corpora, behind a **945-test gate** — and the math spine, the netcode region, the frontfps ladder, and the toric/rigidity/homology detectors carry **14** C99 placements, so
...
- **Phase V — the MMO stage arc (what the direction became).** The work since Phase IV ran the terrain
  ...
  **RAN-0** (`rannull`: authority nullity — ownership and independence composed into a proof of
  ...
  and the **standing lease** (`lease`: RAN-0's temporal extension — proof as an interval; state-free
  validity, interval commutation, amortized == reproved, self-expiry + renewal, the lost-update law —
  optimistic distributed admission with proofs instead of locks). The narrative arc so far:
  ...
  declared arc ahead: the authority algebra (difference/projection/closure), n-way nullity and lease
  scheduling, live authority migration; the queued next rung is the N2-window/durable-window
  unification.
