...
class TheSweepSeparatesLaws(unittest.TestCase):
    ...
    def test_generation_beats_declaration(self):
        self.assertEqual(len(EA.severance_candidates()), 74)
    ...
    def test_the_inert_share_is_strictly_between_zero_and_all(self):
        """46 of 74 taught nothing, and that is first-class. A sweep where everything mattered would
        ...
        `admit_event_for_world` off the whole-log path. It is still a live door (`worldpeer` calls
        it, `netcode-world-admits` gates it) and no behaviour changed — it is simply no longer
        ...
        self.assertEqual((inert, total), (46, 74))
...
class TheMemos(unittest.TestCase):
    ...
    THAT 73 IS A RECORD, NOT A COUNT, and is deliberately left alone as the sweep grows (L64):
    591 calls and 8.1x were observed together on one run, and re-pointing the denominator at a
    later candidate set would manufacture a ratio nobody measured."""
