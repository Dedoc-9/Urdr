...
**203 suites**, discovered automatically by [`../verify.py`](../verify.py) (also runnable with
...
- **Movement & observers (MMO Stages A–B)** — `test_stance`, `test_gaze`, `test_drive`,
  ...
  `test_lift` (how much of a certified identity survives being carried into a richer
  representation — five counts never fused, and a proposed exponential refuted at its premise),
  `test_vantage` (the first-person frame — the eye taken not derived, the jump cycle closing
  bit-identically in pixels, and the compass law that found the yaw table mirrored),
  `test_framing` (does this world fit in this frame — the coverage prediction `horizon_row`
  could not make, checked against all three framing failures this repo produced),
  `test_vouch` (can rollback reproduce the exact REASON the actor was grounded — a
  mid-trajectory resume, and a divergence report that names a cell), `test_retain` (what a
  snapshot must KEEP — an ablation sweep in which INERT is never read as redundancy),
  `test_mould` (the record takes the SHAPE of the state — derived not tagged, and the wrong
  mould caught by refusal rather than by a replay that silently diverges), `test_measure`
  (a performance claim is valid only when its workload, host, denominator and baseline are
  named — and the op-count result that needed no stopwatch), `test_rollbench` (the instrument
  `measure` could not contain — a sealed host log, and a harness that refuses to grade its own
  ...
  `test_repeat` (variance has levels: 200 iterations in one process sample exactly one, so every
  quantile this tree has reported is UNDETERMINED until a second execution exists), `test_deeper`
  (a timing difference with no counted difference is UNEXPLAINED — Ousterhout's rule against the
  ticks-8 band the op model says cannot exist).
...
actually bites. Together they are the `unit-falsifiers` row of the gate: **2748 unit falsifiers, 0 red**.
