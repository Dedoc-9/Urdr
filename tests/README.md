...
**113 suites**, discovered automatically by [`../verify.py`](../verify.py) (also runnable with
...
- **MMO Stages H/I (durability, recovery, streaming, the regional cut, the mutable world)** —
  ...
  the interest filter sound and necessary-with-detection, the client as verifier, refuse purity), `test_storm` (W2, the deterministic adversarial-transport loom — convergence-under-chaos, typed chaos with the primary-reorder floor, the prefix property under loss, malice-under-chaos, the becalmed control).
...
actually bites. Together they are the `unit-falsifiers` row of the gate: **995 unit falsifiers, 0 red**.
